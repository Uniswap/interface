...
import { PermitTransferFrom } from "@taraswap/permit2-sdk";
...
type DutchAuctionOrderResponse =
  ...
  | V2DutchAuctionOrderSuccess;

function isV2DutchAuctionOrderSuccess(
  response: any
): response is V2DutchAuctionOrderSuccess {
      ...
      trace({ name: "Swap (Dutch)", op: "swap.x.dutch" }, async (trace) => {
        ...
        try {
          ...
          if (isErrorResponse(res, responseBody)) {
            ...
            throw new Error(
              `${
                responseBody.errorCode ?? responseBody.detail ?? "Unknown error"
              }`
            );
          }
          ...
          return {
            ...
            response: {
              orderHash: isV2DutchAuctionOrderSuccess(responseBody)
                ? responseBody.orderHash
                : responseBody.hash,
          ...
          if (error instanceof UserRejectedRequestError) {
            ...
            error instanceof SignatureExpiredError ||
            error instanceof UniswapXv2HardQuoteError
          ) {
