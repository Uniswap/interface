...
import { USDC, USDC_X1_TestNet } from '../constants/tokens'

// Stablecoin amounts used when calculating spot price for a given currency.
// The amount is large enough to filter low liquidity pairs.
export const STABLECOIN_AMOUNT_OUT: { [key in SupportedInterfaceChain]: CurrencyAmount<Token> } = {
  [ChainId.X1]: CurrencyAmount.fromRawAmount(USDC, 100_000e6),
  [ChainId.X1_TESTNET]: CurrencyAmount.fromRawAmount(USDC_X1_TestNet, 100_000e6),
}
...
export default function useStablecoinPrice(currency?: Currency): Price<Currency, Token> | undefined {
  ...
  const { trade } = useRoutingAPITrade(false /* skip */, TradeType.EXACT_OUTPUT, amountOut, currency, INTERNAL_ROUTER_PREFERENCE_PRICE)
  const price = useMemo(() => {
