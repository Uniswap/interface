...
import { NEVER_RELOAD } from "@taraswap/redux-multicall";
...
export default function VotePage() {
  ...
  const startDate = getDateFromBlock(
    ...
    currentBlock,
    (account.chainId && AVERAGE_BLOCK_TIME_IN_SECS[account.chainId]) ??
      DEFAULT_AVERAGE_BLOCK_TIME_IN_SECS,
    currentTimestamp
  );
  ...
  const showExecuteButton =
    account.isConnected && proposalData?.status === ProposalState.QUEUED;
  ...
  const linkIfAddress = (content: string) => {
    if (isAddress(content) && account.chainId) {
      const commonName =
        COMMON_CONTRACT_NAMES[account.chainId]?.[content] ?? content;
      return (
        <ExternalLink
