...
// const PROTOCOL_VERSIONS = [ProtocolVersion.V4, ProtocolVersion.V3, ProtocolVersion.V2, ProtocolVersion.Fewv2]
const PROTOCOL_VERSIONS = [ProtocolVersion.V2, ProtocolVersion.Fewv2]
...
export function PositionsHeader({
  ...
  // - Prefer the explicitly selected chain filter
  // - Fallback to the sole enabled chain when only one chain is active
  const effectiveChain: UniverseChainId | null =
    selectedChain ?? (chains.length === 1 ? (chains[0] as UniverseChainId | undefined) ?? null : null)
