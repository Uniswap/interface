...
export default function FewV2PositionPage() {
  const chainId = useChainIdFromUrlParam()

  return (
    <MultichainContextProvider initialChainId={chainId}>
      <FewV2PositionPageInner />
    </MultichainContextProvider>
  )
}

function FewV2PositionPageInner() {
  ...
  const { data, isLoading: positionLoading, refetch } = useGetPositionQuery(queryParams)
  ...
  // Query Fewv2 pair chain data
  // const [, fewPair] = useFewV2Pair(positionInfo?.currency0Amount.currency, positionInfo?.currency1Amount.currency)
        ...
        <Flex gap="$gap20" width="100%">
          ...
          {isOwner && (
            <Flex row gap="$gap12" alignItems="center" maxWidth="100%" flexWrap="wrap">
              {/* Temporarily disabled migrate to v3 functionality */}
              {/* <Button
                size="small"
                emphasis="secondary"
                $sm={{ width: '100%' }}
                ...
                onPress={() => {
                  if (fewPair && chainId && pairAddress && !savedSerializedPairs[chainId]?.[pairAddress]) {
                    addFewPair(fewPair)
                  }
                  navigate('/migrate/fewv2', {
                    state: {
