import { Pair as FewPair, getFewTokenFromOriginalToken } from '@ring-protocol/few-v2-sdk'
...
import { CreatePositionRouteVariant } from 'pages/Pool/Positions/create/routeVariant'
import {
  CreateFewV2PositionInfo,
  CreatePositionInfo,
...
export function getCurrencyWithWrap(currency: OptionalCurrency, protocolVersion: ProtocolVersion): OptionalCurrency
export function getCurrencyWithWrap(
  currency: OptionalCurrency,
  protocolVersion: ProtocolVersion,
): Currency | Token | undefined {
...
export function poolEnabledProtocolVersion(
  ...
  const v = toClientProtocolVersion(protocolVersion)
  return v === ProtocolVersion.V3 || v === ProtocolVersion.V4
}

export function pairEnabledProtocolVersion(
  protocolVersion: ProtocolVersion | GraphQLProtocolVersion,
): protocolVersion is ProtocolVersion.V2 | ProtocolVersion.Fewv2 {
  const v = toClientProtocolVersion(protocolVersion)
  return v === ProtocolVersion.V2 || v === ProtocolVersion.Fewv2
}
...
export function getFewPairFromPositionStateAndRangeState({
  derivedPositionInfo,
  derivedPriceRangeInfo,
}: {
  derivedPositionInfo: CreatePositionInfo
  ...
  if (derivedPositionInfo.creatingPoolOrPair) {
    return derivedPriceRangeInfo.protocolVersion === ProtocolVersion.Fewv2 ? derivedPriceRangeInfo.mockPair : undefined
  }

  return derivedPositionInfo.protocolVersion === ProtocolVersion.Fewv2 ? derivedPositionInfo.pair : undefined
}
...
export function getPoolFromPositionStateAndRangeState({
  ...
  if (
    ...
    derivedPositionInfo.protocolVersion === ProtocolVersion.Fewv2
  ) {
    return undefined
  }
...
function createMockFewPair(price?: Price<Currency, Currency>) {
  if (price) {
    const quoteFewToken = getFewTokenFromOriginalToken(price.quoteCurrency.wrapped, price.quoteCurrency.chainId)
    const baseFewToken = getFewTokenFromOriginalToken(price.baseCurrency.wrapped, price.baseCurrency.chainId)

    return new FewPair(
      CurrencyAmount.fromRawAmount(quoteFewToken, price.numerator),
      CurrencyAmount.fromRawAmount(baseFewToken, price.denominator),
    )
  } else {
    return undefined
  }
...
export function getDependentAmountFromFewV2Pair({
  independentAmount,
  otherAmount,
  pair,
  exactField,
  ...
  const fewToken0Wrapped = inputsAreFewTokens
    ? token0Wrapped
    : getFewTokenFromOriginalToken(token0Wrapped, token0Wrapped.chainId)
  const fewToken1Wrapped = inputsAreFewTokens
    ? token1Wrapped
    : getFewTokenFromOriginalToken(token1Wrapped, token1Wrapped.chainId)
  const fewIndependentCurrency = inputsAreFewTokens
    ? independentAmount.currency.wrapped
    : getFewTokenFromOriginalToken(independentAmount.currency.wrapped, independentAmount.currency.chainId)
  const fewIndependentAmount = CurrencyAmount.fromRawAmount(fewIndependentCurrency, independentAmount.quotient)

  try {
    const dependentTokenAmount =
      exactField === PositionField.TOKEN0
        ? pair.priceOf(fewToken0Wrapped).quote(fewIndependentAmount.wrapped)
        : pair.priceOf(fewToken1Wrapped).quote(fewIndependentAmount.wrapped)
...
export function getFewV2PriceRangeInfo({
  state,
  derivedPositionInfo,
}: {
  state: PriceRangeState
  derivedPositionInfo: CreateFewV2PositionInfo
}): FewV2PriceRangeInfo {
  const { currencies } = derivedPositionInfo
  const [baseCurrency] = getInvertedTuple(currencies, state.priceInverted)

  const baseToken = getCurrencyWithWrap(baseCurrency, ProtocolVersion.Fewv2)
  const sortedTokens = getSortedCurrenciesTuple(
    getCurrencyWithWrap(currencies[0], ProtocolVersion.Fewv2),
    getCurrencyWithWrap(currencies[1], ProtocolVersion.Fewv2),
  )
  const priceDifference = getPriceDifference({
    initialPrice: state.initialPrice,
    defaultInitialPrice: derivedPositionInfo.defaultInitialPrice,
  ...
  return {
    ...
    price,
    mockPair: createMockFewPair(price),
    deposit0Disabled: false,
    deposit1Disabled: false,
    invertPrice,
    priceDifference,
  } satisfies FewV2PriceRangeInfo
}
...
export function generateAddLiquidityApprovalParams({
  ...
  return withFewTokenRouteVariant(request, positionState.routeVariant) as CheckApprovalLPRequest
}

function getFewV2OriginCurrencyIndexForPoolToken({
  unsortedCurrencies,
  sortedToken,
}: {
  unsortedCurrencies: [Currency, Currency]
  sortedToken: Currency
}): PositionField | undefined {
  const index = unsortedCurrencies.findIndex((currency) => {
    const wrappedCurrency = currency.wrapped
    const fewToken = getFewTokenFromOriginalToken(wrappedCurrency, wrappedCurrency.chainId)

    return sortedToken.wrapped.equals(wrappedCurrency) || sortedToken.wrapped.equals(fewToken)
  })
...
function getIndependentToken({
  ...
  let token0Index: PositionField
  let token1Index: PositionField
  ...
  if (
    ...
    if (chainId) {
      // Existing FewV2 pairs use FewTokens, while newly-created mock pairs used by the
      // price range flow may still carry original wrapped tokens.
      token0Index =
        getFewV2OriginCurrencyIndexForPoolToken({ unsortedCurrencies, sortedToken: sortedToken0 }) ??
        PositionField.TOKEN0
      token1Index = token0Index === PositionField.TOKEN0 ? PositionField.TOKEN1 : PositionField.TOKEN0
    } else {
      // Fallback to original logic
      token0Index = tokenA && sortedToken0.equals(tokenA) ? PositionField.TOKEN0 : PositionField.TOKEN1
      token1Index = tokenB && sortedToken1.equals(tokenB) ? PositionField.TOKEN1 : PositionField.TOKEN0
    }
  } else {
    // For V2/V3/V4: sortedToken0/sortedToken1 match tokenA/tokenB directly
    token0Index = tokenA && sortedToken0.equals(tokenA) ? PositionField.TOKEN0 : PositionField.TOKEN1
    token1Index = tokenB && sortedToken1.equals(tokenB) ? PositionField.TOKEN1 : PositionField.TOKEN0
  }
...
function getPoolTokenAddressForTradingApi({
  unsortedCurrencies,
  sortedToken,
  protocolVersion,
  routeVariant,
  ...
  if (protocolVersion === ProtocolVersion.Fewv2 && routeVariant !== CreatePositionRouteVariant.FewToken) {
    const originCurrencyIndex = getFewV2OriginCurrencyIndexForPoolToken({ unsortedCurrencies, sortedToken })
    const originCurrency =
      originCurrencyIndex === PositionField.TOKEN0
        ? unsortedCurrencies[0]
        : originCurrencyIndex === PositionField.TOKEN1
          ? unsortedCurrencies[1]
          : undefined

    return getCurrencyAddressForTradingApi(originCurrency ?? sortedToken)
  }

  const matchingCurrency = unsortedCurrencies.find((currency) => {
    const currencyForPool = getCurrencyWithWrap(currency, protocolVersion)
    return currencyForPool?.equals(sortedToken)
  })

  return getCurrencyAddressForTradingApi(matchingCurrency ?? sortedToken)
}

function withFewTokenRouteVariant<T extends object>(
  request: T,
  routeVariant?: CreatePositionRouteVariant,
): T & { routeVariant?: 'FEWTOKEN' } {
  if (routeVariant !== CreatePositionRouteVariant.FewToken) {
    return request
  }

  return { ...request, routeVariant: 'FEWTOKEN' }
}
...
export function generateCreateCalldataQueryParams({
  ...
  if (
    ...
    const { independentToken } = getIndependentToken({
      unsortedCurrencies: currencies,
      sortedToken0,
      sortedToken1,
      independentField,
      protocolVersion: derivedPositionInfo.protocolVersion,
      routeVariant: positionState.routeVariant,
    })
    ...
    const token0Address = getPoolTokenAddressForTradingApi({
      unsortedCurrencies: currencies,
      sortedToken: sortedToken0,
      protocolVersion: derivedPositionInfo.protocolVersion,
      routeVariant: positionState.routeVariant,
    })
    const token1Address = getPoolTokenAddressForTradingApi({
      unsortedCurrencies: currencies,
      sortedToken: sortedToken1,
      protocolVersion: derivedPositionInfo.protocolVersion,
      routeVariant: positionState.routeVariant,
    })
    ...
    return withFewTokenRouteVariant(request, positionState.routeVariant) as CreateLPPositionRequest
  }
  ...
  return {
    ...
    position: {
      ...
      pool: {
        tickSpacing,
        token0: getPoolTokenAddressForTradingApi({
          unsortedCurrencies: currencies,
          sortedToken: sortedToken0,
          protocolVersion: derivedPositionInfo.protocolVersion,
          routeVariant: positionState.routeVariant,
        }),
        token1: getPoolTokenAddressForTradingApi({
          unsortedCurrencies: currencies,
          sortedToken: sortedToken1,
          protocolVersion: derivedPositionInfo.protocolVersion,
          routeVariant: positionState.routeVariant,
...
function toClientProtocolVersion(version: ProtocolVersion | GraphQLProtocolVersion): ProtocolVersion {
  if (typeof version === 'number') {
    return version as ProtocolVersion
  }
  switch (version) {
    case GraphQLProtocolVersion.V2:
      return ProtocolVersion.V2
    case GraphQLProtocolVersion.V3:
      return ProtocolVersion.V3
    case GraphQLProtocolVersion.V4:
      return ProtocolVersion.V4
    case GraphQLProtocolVersion.Fewv2:
      return ProtocolVersion.Fewv2
    default:
      return ProtocolVersion.V4
  }
}
