...
import FewTokenCurrencySearchModal from 'pages/Pool/Positions/create/FewTokenCurrencySearchModal'
...
import { useDefaultFewTokenActiveTokensForChainIds } from 'state/lists/hooks'
...
function createFallbackCurrencyInfo(currency?: Currency, listToken?: Token): Maybe<CurrencyInfo> {
  if (!currency) {
    return undefined
  }

  const tokenWithLogo = listToken as (Token & { logoURI?: string }) | undefined
...
export function SelectTokensStep({
  ...
  const { formatPercent } = useFormatter()
  const { t } = useTranslation()
  const { setSelectedChainId, setIsUserSelectedToken, chainId: multichainChainId } = useMultichainContext()
  const trace = useTrace()
  const [hookModalOpen, setHookModalOpen] = useState(false)
  const isLpIncentivesEnabled = useFeatureFlag(FeatureFlags.LpIncentives)

  const {
    positionState: { hook, userApprovedHook, currencyInputs, fee, protocolVersion, routeVariant },
    setPositionState,
    derivedPositionInfo,
  ...
  // Set isUserSelectedToken to true if we have a chainId from multichain context,
  // so the token selector uses the correct chain (e.g., X Layer for V2)
  useEffect(() => {
    if (multichainChainId) {
      setIsUserSelectedToken(true)
      setSelectedChainId(multichainChainId)
    }
  }, [multichainChainId, setIsUserSelectedToken, setSelectedChainId])
  ...
  const continueButtonEnabled =
    ...
    (derivedPositionInfo.protocolVersion === ProtocolVersion.Fewv2 && derivedPositionInfo.pair) ||
  ...
  const supportedChains = useMemo(() => {
    if (protocolVersion === ProtocolVersion.V4) {
      return chains.filter((chain) => !isV4UnsupportedChain(chain))
    }

    if (protocolVersion === ProtocolVersion.V2) {
      // For V2, only show networks that don't support FewToken (e.g., X Layer) and support positions
      return chains.filter(
        (chain) =>
          SUPPORTED_V2POOL_CHAIN_IDS.includes(chain) &&
          NETWORKS_WITHOUT_FEWTOKEN.includes(chain) &&
          !POSITIONS_UNSUPPORTED_CHAINS.includes(chain),
      )
    }

    if (protocolVersion === ProtocolVersion.Fewv2) {
      // Filter out networks that don't support FewToken (e.g., X Layer) or have no positions support
      return chains.filter(
        (chain) =>
          SUPPORTED_V2POOL_CHAIN_IDS.includes(chain) &&
          !NETWORKS_WITHOUT_FEWTOKEN.includes(chain) &&
  ...
  const isFewTokenVariant = routeVariant === CreatePositionRouteVariant.FewToken
  const fewTokenListTokens = useDefaultFewTokenActiveTokensForChainIds(supportedChains)
  const fewTokenListTokenMap = useMemo(
    () =>
      fewTokenListTokens.reduce<Record<string, Token>>((acc, token) => {
  ...
  const token0CurrencyInfo =
    useCurrencyInfo(currencyId(token0)) ??
    (isFewTokenVariant
      ? createFallbackCurrencyInfo(
          token0,
          token0?.isToken ? fewTokenListTokenMap[`${token0.chainId}:${token0.address.toLowerCase()}`] : undefined,
        )
      : undefined)
  const token1CurrencyInfo =
    useCurrencyInfo(currencyId(token1)) ??
    (isFewTokenVariant
      ? createFallbackCurrencyInfo(
          token1,
          token1?.isToken ? fewTokenListTokenMap[`${token1.chainId}:${token1.address.toLowerCase()}`] : undefined,
        )
      : undefined)
      ...
      <PrefetchBalancesWrapper>
        <Container {...rest}>
          ...
          <Flex gap="$spacing24">
            <Flex>
              ...
              <Text variant="body3" color="$neutral2">
                {protocolVersion === ProtocolVersion.V2 || protocolVersion === ProtocolVersion.Fewv2
                  ? t('fee.tier.description.v2')
        ...
        {isFewTokenVariant ? (
          ...
          <CurrencySearchModal
            ...
            onCurrencySelect={handleCurrencySelect}
            chainIds={supportedChains}
            includeAllNetworks={protocolVersion !== ProtocolVersion.V2 && protocolVersion !== ProtocolVersion.Fewv2}
          />
        )}
      </PrefetchBalancesWrapper>
    </>
