...
import {
  ...
  getCurrencyWithWrap,
  getDependentAmountFromFewV2Pair,
  getDependentAmountFromV2Pair,
  getDependentAmountFromV3Position,
  getDependentAmountFromV4Position,
  getFewPairFromPositionStateAndRangeState,
  getFewV2PriceRangeInfo,
...
export function useDerivedPositionInfo(state: PositionState): CreatePositionInfo {
  ...
  //Few
  const fewV2PairResult = useFewV2Pair(sortedTokens?.[0], sortedTokens?.[1], {
    inputsAreFewTokens: routeVariant === CreatePositionRouteVariant.FewToken,
  })
  const fewV2PairIsLoading = fewV2PairResult[0] === PairState.LOADING

  const fewV2Pair = pairsQueryEnabled ? fewV2PairResult[1] || undefined : undefined

  // console.log('fewV2PairResult', fewV2PairResult)
  ...
  const creatingPoolOrPair = useMemo(() => {
    ...
    if (protocolVersion === ProtocolVersion.Fewv2) {
      return fewV2PairResult[0] === PairState.NOT_EXISTS
    }
  ...
  }, [protocolVersion, poolData?.pools, pairResult, fewV2PairResult, v3PoolResult])
  ...
  return useMemo(() => {
    ...
    if (protocolVersion === ProtocolVersion.Fewv2) {
      return {
        ...
        pair: fewV2Pair,
        creatingPoolOrPair,
        poolOrPairLoading: fewV2PairIsLoading,
        isPoolOutOfSync,
        defaultInitialPrice,
        isDefaultInitialPriceLoading,
        refetchPoolData,
      } satisfies CreateFewV2PositionInfo
    }
...
export function useDerivedPriceRangeInfo(state: PriceRangeState): PriceRangeInfo {
  ...
  const priceRangeInfo = useMemo(() => {
    ...
    if (derivedPositionInfo.protocolVersion === ProtocolVersion.Fewv2) {
      return getFewV2PriceRangeInfo({ state, derivedPositionInfo })
    }
...
export function useDerivedDepositInfo(state: DepositState): DepositInfo {
  ...
  const depositInfoProps: UseDepositInfoProps = useMemo(() => {
    ...
    if (protocolVersion === ProtocolVersion.Fewv2) {
      return {
        protocolVersion,
        pair: getFewPairFromPositionStateAndRangeState({ derivedPositionInfo, derivedPriceRangeInfo }),
        address: account.address,
        token0: derivedPositionInfo.currencies[0],
        token1: derivedPositionInfo.currencies[1],
        inputsAreFewTokens: positionState.routeVariant === CreatePositionRouteVariant.FewToken,
        exactField,
        exactAmounts,
      } satisfies UseDepositInfoProps
    }
...
export function useDepositInfo(state: UseDepositInfoProps): DepositInfo {
  ...
  const dependentAmount: CurrencyAmount<Currency> | undefined = useMemo(() => {
    ...
    if (protocolVersion === ProtocolVersion.Fewv2) {
      return getDependentAmountFromFewV2Pair({
        independentAmount,
        otherAmount,
        pair: state.pair,
        exactField,
        ...
        token1,
        dependentToken,
        inputsAreFewTokens: state.inputsAreFewTokens,
      })
    }
