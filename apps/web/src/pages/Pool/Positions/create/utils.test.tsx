...
import { USDM_MEGAETH_MAINNET, nativeOnChain } from 'uniswap/src/constants/tokens'
import { IndependentToken } from 'uniswap/src/data/tradingApi/__generated__'
import { AccountType } from 'uniswap/src/features/accounts/types'
import { UniverseChainId } from 'uniswap/src/features/chains/types'
import { DAI, ETH, WETH } from 'uniswap/src/test/fixtures/lib'
...
describe('generateCreateCalldataQueryParams', () => {
  it('keeps MegaETH V2 native token0 amounts aligned when UI order differs from pool order', () => {
    const megaEth = nativeOnChain(UniverseChainId.MEGAETHMainnet)
    const ethAmount = '10000000000000000'
    const usdmAmount = '22929718000000000000'
    const pair = new Pair(
      CurrencyAmount.fromRawAmount(megaEth.wrapped, ethAmount),
      CurrencyAmount.fromRawAmount(USDM_MEGAETH_MAINNET, usdmAmount),
    )

    const positionState: PositionState = {
      protocolVersion: ProtocolVersion.V2,
      currencyInputs: {
        [PositionField.TOKEN0]: USDM_MEGAETH_MAINNET,
        [PositionField.TOKEN1]: megaEth,
      },
      fee: {
    ...
    const derivedPositionInfo: CreateV2PositionInfo = {
      protocolVersion: ProtocolVersion.V2,
      currencies: [USDM_MEGAETH_MAINNET, megaEth],
      creatingPoolOrPair: false,
      pair,
      isPoolOutOfSync: false,
      refetchPoolData: () => undefined,
    }
    const derivedDepositInfo: DepositInfo = {
      currencyAmounts: {
        [PositionField.TOKEN0]: CurrencyAmount.fromRawAmount(USDM_MEGAETH_MAINNET, usdmAmount),
        [PositionField.TOKEN1]: CurrencyAmount.fromRawAmount(megaEth, ethAmount),
      },
    }

    const request = generateCreateCalldataQueryParams({
      account: { type: AccountType.Readonly, address: '0x0000000000000000000000000000000000000001' },
      positionState,
      derivedPositionInfo,
      priceRangeState: {
        fullRange: true,
    ...
    expect(request).toMatchObject({
      ...
      independentAmount: ethAmount,
      defaultDependentAmount: usdmAmount,
      position: {
        pool: {
          token0: ZERO_ADDRESS,
          token1: USDM_MEGAETH_MAINNET.address,
        },
      },
    })
  })

  it('keeps MegaETH FewV2 native token0 in the payload instead of generating USDm/USDm', () => {
    const megaEth = nativeOnChain(UniverseChainId.MEGAETHMainnet)
    const ethAmount = '10000000000000000'
    const usdmAmount = '22930000000000000000'
    const mockPairWithOriginalTokens = new FewPair(
      CurrencyAmount.fromRawAmount(megaEth.wrapped, ethAmount),
      CurrencyAmount.fromRawAmount(USDM_MEGAETH_MAINNET, usdmAmount),
    )
    ...
    const positionState: PositionState = {
      ...
      currencyInputs: {
        [PositionField.TOKEN0]: megaEth,
        [PositionField.TOKEN1]: USDM_MEGAETH_MAINNET,
      },
      fee: {
        feeAmount: FeeAmount.MEDIUM,
        tickSpacing: TICK_SPACINGS[FeeAmount.MEDIUM],
    ...
    const derivedPositionInfo: CreateFewV2PositionInfo = {
      protocolVersion: ProtocolVersion.Fewv2,
      currencies: [megaEth, USDM_MEGAETH_MAINNET],
      creatingPoolOrPair: true,
      isPoolOutOfSync: false,
      refetchPoolData: () => undefined,
    }
    const derivedDepositInfo: DepositInfo = {
      currencyAmounts: {
        [PositionField.TOKEN0]: CurrencyAmount.fromRawAmount(megaEth, ethAmount),
        [PositionField.TOKEN1]: CurrencyAmount.fromRawAmount(USDM_MEGAETH_MAINNET, usdmAmount),
      },
    }
    ...
    const request = generateCreateCalldataQueryParams({
      ...
      derivedPriceRangeInfo: {
        protocolVersion: ProtocolVersion.Fewv2,
        mockPair: mockPairWithOriginalTokens,
        deposit0Disabled: false,
        deposit1Disabled: false,
        invertPrice: false,
      },
