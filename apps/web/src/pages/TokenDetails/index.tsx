...
import { gqlToCurrency, gqlToRingCurrency } from 'appGraphql/data/util'
...
import { FEW_WRAPPED_NATIVE_CURRENCY, nativeOnChain } from 'uniswap/src/constants/tokens'
...
function useTDPRingCurrency(
  ...
  const queryCurrency = useMemo(() => {
    ...
    if (tokenQuery?.token) {
      return gqlToRingCurrency(tokenQuery.token)
    }
    return undefined
  }, [isNative, currencyChainId, tokenQuery.token])
  // fetches on-chain token if query data is missing and page chain matches global chain (else fetch won't work)
  const skipOnChainFetch = Boolean(queryCurrency) || currencyChainId !== appChainId
  const onChainToken = useOnChainToken(tokenAddress, currencyChainId, skipOnChainFetch)
  const currency = queryCurrency ?? onChainToken
...
function useCreateTDPContext(): PendingTDPContext | LoadedTDPContext {
  ...
  // tokenAddress from URL is origintoken address (for display in URL, not changed)
  const originTokenAddress = isNative ? getNativeTokenDBAddress(currencyChainInfo.backendChain.chain) : tokenAddress

  // Get origintoken and fewtoken from location state (passed from TokenTable via linkState)
  const locationState = (useLocation().state as { preloadedLogoSrc?: string; token?: string; fewtoken?: string }) ?? {}
  const fewTokenFromState = locationState.fewtoken

  // Determine fewtoken address: from location state, or calculate using getFewTokenAddress
  const fewTokenAddress = useMemo(() => {
    // If fewtoken is in location state, use it directly
    if (fewTokenFromState) {
      return fewTokenFromState
    }

    // If native token, use FEW wrapped native currency address
    if (isNative) {
      return FEW_WRAPPED_NATIVE_CURRENCY[currencyChainInfo.id]?.address
    }

    // Use origintoken from location state if available, otherwise use URL param
    const originToken = locationState.token || originTokenAddress
    if (!originToken) {
      return tokenAddress
    }

    // Calculate fewtoken address from origintoken address
    try {
      return getFewTokenAddress(originToken, currencyChainInfo.id)
    } catch {
      return originToken
    }
  }, [fewTokenFromState, locationState.token, isNative, originTokenAddress, currencyChainInfo.id, tokenAddress])

  // Use fewtoken address for querying data if available, otherwise use origintoken address
  const tokenDBAddress = fewTokenAddress || originTokenAddress || tokenAddress

  // const tokenQuery = useTokenWebQuery({
  //   variables: { address: tokenDBAddress, chain: currencyChainInfo.backendChain.chain },
  //   errorPolicy: 'all',
  // })
  const tokenQuery = useTokenDetail(tokenDBAddress as string, currencyChainInfo.backendChain.chain)
  ...
  const { currency, currencyWasFetchedOnChain } = useTDPRingCurrency(
    tokenQuery,
    tokenAddress,
  ...
  const { preloadedLogoSrc } = locationState
  const extractedColorSrc = tokenQuery.token?.project?.logoUrl ?? preloadedLogoSrc
  const tokenColor =
    useSrcColor(extractedColorSrc, tokenQuery.token?.name ?? tokenQuery.token?.project?.name, colors.surface2.val)
