  ...
  return (
    <Flex row gap="$spacing24" maxHeight={iconSize} alignItems="flex-start">
      <SocialIcon iconColor="#00C32B">
        <Anchor href="https://github.com/RingProtocol" target="_blank">
          <Github size={iconSize} fill="inherit" />
        </Anchor>
      </SocialIcon>
      <SocialIcon iconColor="#20BAFF">
        <Anchor href="https://x.com/ProtocolRing" target="_blank">
          <Twitter size={iconSize} fill="inherit" />
        </Anchor>
      </SocialIcon>
      <SocialIcon iconColor="#5F51FF">
        <Anchor href="https://discord.com/invite/TefBNDZBQP" target="_blank">
    ...
    <Flex width={130} $md={{ width: '100%' }} flexGrow={0} flexShrink={1} flexBasis="auto" gap={8}>
      ...
      <Flex gap={5}>
        {items.map((item, index) => {
          // Special handling for brand assets download
          if (item.href?.endsWith('.zip')) {
            return (
              <a key={`footer_${title}_${index}}`} href={item.href} download style={{ textDecoration: 'none' }}>
                <Text variant="subheading2" color="$neutral2" hoverStyle={{ opacity: 0.6 }}>
                  {item.label}
                </Text>
              </a>
            )
...
export function Footer() {
  ...
  const brandAssets = {
    ...
    href: 'https://github.com/RingProtocol/brand-assets/raw/main/Ring%20Protocol%20Brand%20Assets.zip',
    internal: false,
  }
  ...
  return (
    <Flex maxWidth="100vw" width="100%" gap="$spacing24" pt="$none" px="$spacing48" pb={40} $lg={{ px: '$spacing40' }}>
      ...
      >
        <Text variant="body3">© {currentYear} - Ring Protocol Labs</Text>
        <Flex row alignItems="center" gap="$spacing16">
          ...
          <Anchor textDecorationLine="none" href={getInternalLinkHref('/terms-of-service')}>
            <PolicyLink>{t('common.termsOfService')}</PolicyLink>
          </Anchor>
          <Anchor textDecorationLine="none" href={getInternalLinkHref('/privacy-policy')}>
            <PolicyLink>{t('common.privacyPolicy')}</PolicyLink>
          </Anchor>
        </Flex>
      </Flex>
    </Flex>
