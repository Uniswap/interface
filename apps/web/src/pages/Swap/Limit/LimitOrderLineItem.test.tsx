...
const AllLineItemsTypes = Object.keys(LimitOrderLineItemType).map(Number).filter(Boolean)
const lineItemProps = {
  syncing: false,
}
  ...
  const { asFragment } = render(
    <>
      {AllLineItemsTypes.map((type) => (
        <LimitOrderLineItem key={type} trade={trade} type={type} {...lineItemProps} {...props} />
      ))}
    </>,
  )
...
describe('LimitOrderLineItem.tsx', () => {
  beforeEach(() => {
  ...
  it('Expiry', () => {
    render(
      <LimitOrderLineItem
        key={LimitOrderLineItemType.EXPIRY}
        trade={LIMIT_ORDER_TRADE}
        type={LimitOrderLineItemType.EXPIRY}
        syncing={false}
      />,
    )
