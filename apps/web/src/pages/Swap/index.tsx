...
const SWAP_TABS = [SwapTab.Swap, SwapTab.Stock, SwapTab.Limit, SwapTab.Send, SwapTab.Buy, SwapTab.Sell]
    ...
    <Flex>
      ...
      {!hideHeader && SWAP_TAB_OPTIONS.length === 1 && SWAP_TAB_OPTIONS[0] && (
        <Flex row gap="$spacing16" px="$spacing12" height={42} alignItems="center">
          {SWAP_TAB_OPTIONS[0].display}
        </Flex>
      )}
      {(currentTab === SwapTab.Swap || currentTab === SwapTab.Stock) && (
        <Flex gap="$spacing16">
      ...
      {isBuyEnabled && currentTab === SwapTab.Buy && BuyForm && (
        <BuyForm
      ...
      {isSellEnabled && currentTab === SwapTab.Sell && BuyForm && (
        <BuyForm
