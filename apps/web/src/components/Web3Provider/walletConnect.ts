...
if (process.env.REACT_APP_WALLET_CONNECT_PROJECT_ID === undefined) {
  throw new Error(
    "REACT_APP_WALLET_CONNECT_PROJECT_ID must be a defined environment variable"
  );
}
...
export interface WalletConnectConnector extends Connector {
  ...
  getProvider(): Promise<{
    modal: {
      setTheme: ({ themeMode }: { themeMode: "dark" | "light" }) => void;
    };
  }>;
}
    ...
    config.emitter.on("message", ({ type, data }) => {
      if (type === "display_uri") {
        ...
        config.emitter.emit("message", {
          type: "display_uniswap_uri",
          data: uniswapWalletUri,
        });
        ...
        if (isWebIOS || isWebAndroid) {
          ...
          window.location.href = `uniswap://wc?uri=${encodeURIComponent(
            data as string
          )}`;
        }
      }
