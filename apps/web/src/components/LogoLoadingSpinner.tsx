...
type StyledRotatingSvgProps = ComponentPropsWithoutRef<typeof StyledRotatingSVG>

export type LogoLoadingSpinnerProps = Omit<StyledRotatingSvgProps, 'viewBox' | 'xmlns' | 'children' | 'size'> & {
  /** Rendered width/height (e.g. `"16px"`, `"64px"`). */
  size?: string
  /** Solid fill for ring paths; defaults to theme neutral3. */
  color?: string
}

/**
 * Large orbiting ring spinner (54×54 viewBox) for modal headers, swap confirmation,
 * and other logo-sized loading slots. Not the same as {@link SpinningLoader} from `ui`.
 *
 * Used from limit swap flows and account activity (e.g. cancel orders) — keep this
 * path shared rather than under `features/Swap` only.
 */
export function LogoLoadingSpinner({ size = '16px', color, ...rest }: LogoLoadingSpinnerProps): JSX.Element {
