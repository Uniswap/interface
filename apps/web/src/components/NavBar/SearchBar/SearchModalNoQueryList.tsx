import { useTrendingPoolsCurrencyInfos, useTrendingTokensCurrencyInfos } from 'appGraphql/data/ring/SearchRingWeb'
...
function useSectionsForNoQuerySearch({
  ...
  } = useTrendingTokensCurrencyInfos(chainFilter ?? UniverseChainId.Mainnet)
  ...
  } = useTrendingPoolsCurrencyInfos(chainFilter ?? UniverseChainId.Mainnet)
  const trendingPoolSection = useOnchainItemListSection({
    sectionKey: OnchainItemSectionName.TrendingPools,
    options: pools?.slice(0, numberOfTrendingTokens),
  })
  ...
  return useMemo((): GqlResult<OnchainItemSection<SearchModalOption>[]> => {
    ...
    switch (activeTab) {
      ...
      case SearchTab.Pools:
        sections = [...(recentSearchSection ?? []), ...(trendingPoolSection ?? [])] // ...getMockTrendingPoolsSection(15)
        return {
          data: sections,
          loading: loadingPools,
          error: poolsError,
