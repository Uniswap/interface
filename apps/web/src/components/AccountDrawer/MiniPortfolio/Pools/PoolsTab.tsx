...
function getPositionKey(position: PositionInfo) {
  ...
  if (position.version === ProtocolVersion.V2 || position.version === ProtocolVersion.Fewv2) {
...
export default function Pools({ account }: { account: string }) {
  ...
  const { data, isLoading } = useGetPositionsQuery({
    ...
    protocolVersions: [ProtocolVersion.V2, ProtocolVersion.V3, ProtocolVersion.V4, ProtocolVersion.Fewv2],
    includeHidden: true,
  })
