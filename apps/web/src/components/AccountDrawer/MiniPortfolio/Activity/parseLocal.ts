...
async function parseFewWrap(
  ...
  let input: Currency | undefined
  let output: Currency | undefined
  if (wrap.inputCurrencyId) {
    input = await getCurrency(currencyIdToAddress(wrap.inputCurrencyId), chainId)
  }
  if (wrap.outputCurrencyId) {
    output = await getCurrency(currencyIdToAddress(wrap.outputCurrencyId), chainId)
  }

  // Fallback if currencies are not found or not provided (legacy txs)
  if (!input) {
    input = wrap.unwrapped ? wrapped : native
  }
  if (!output) {
    output = wrap.unwrapped ? native : wrapped
  }

  // For few tokens (e.g., fwusdt), use the original token's decimals to display the amount
  // because few tokens and their original tokens have a 1:1 relationship and should use the same decimals
  // The currencyAmountRaw is calculated based on the original token's decimals
  // Note: getCurrency may return incorrect decimals for few tokens if GraphQL data is missing,
  // so we always use the original token's decimals for few tokens
  let inputForDisplay = input
  let outputForDisplay = output
  if (input && output && input.isToken && output.isToken) {
    const inputToken = input as Token
    const outputToken = output as Token
    const isInputFewToken = isFewToken(inputToken)
    const isOutputFewToken = isFewToken(outputToken)

    // Case 1: FewWrap - input is few token, output is original token
    // Always use output's decimals for input display (regardless of what GraphQL returned)
    if (isInputFewToken && !isOutputFewToken) {
      inputForDisplay = new Token(
        inputToken.chainId,
        inputToken.address,
    ...
    // Case 2: FewUnwrap - input is original token, output is few token
    // Always use input's decimals for output display (regardless of what GraphQL returned)
    if (!isInputFewToken && isOutputFewToken) {
      outputForDisplay = new Token(
        outputToken.chainId,
        outputToken.address,
        inputToken.decimals,
        outputToken.symbol,
...
function createFallbackToken(
  ...
  return new Token(chainId, address, decimals, symbol, name ?? symbol)
}

async function getActivityCurrency(
  address: string,
  chainId: UniverseChainId,
  decimals?: number,
  symbol?: string,
  ...
  return (await getCurrency(address, chainId)) ?? createFallbackToken(address, chainId, decimals, symbol, name)
}
...
function formatAmountWithSymbol(
  ...
  const formattedAmount = formatNumber({
    input: parseFloat(CurrencyAmount.fromRawAmount(currency, amountRaw).toSignificant()),
    type: NumberType.TokenNonTx,
  })

  return currency.symbol ? `${formattedAmount} ${currency.symbol}` : formattedAmount
}
...
async function parseApproval(
  ...
  const currency = await getActivityCurrency(
...
async function parseMorphoMarket(
  info:
    | MorphoSupplyTransactionInfo
    | MorphoBorrowTransactionInfo
    | MorphoRepayTransactionInfo
    | MorphoMarketWithdrawTransactionInfo,
  chainId: UniverseChainId,
  status: TransactionStatus,
  formatNumber: FormatNumberFunctionType,
): Promise<Partial<Activity>> {
  ...
  const amountWithSymbol = formatAmountWithSymbol(info.assetAmountRaw, currency, formatNumber)

  return {
    title: getActivityTitle(info.type, status),
    descriptor: i18n.t('activity.transaction.lending.market.descriptor', {
      amountWithSymbol,
      marketLabel: info.marketTitle,
    }),
    currencies: [currency],
  }
}

async function parseMorphoVault(
  info: MorphoVaultDepositTransactionInfo | MorphoVaultWithdrawTransactionInfo,
  chainId: UniverseChainId,
  status: TransactionStatus,
  formatNumber: FormatNumberFunctionType,
): Promise<Partial<Activity>> {
  ...
  return {
    title: getActivityTitle(info.type, status),
    descriptor: i18n.t('activity.transaction.lending.vault.descriptor', {
      amountWithSymbol,
      vaultLabel: info.vaultTitle,
    }),
    currencies: [currency],
  }
}

async function parseMorphoVaultRedeem(
  info: MorphoVaultRedeemTransactionInfo,
  chainId: UniverseChainId,
  status: TransactionStatus,
  formatNumber: FormatNumberFunctionType,
): Promise<Partial<Activity>> {
  const shareCurrency =
    createFallbackToken(info.vaultAddress, chainId, info.shareDecimals, i18n.t('common.shares'), info.vaultTitle) ??
    undefined
  const assetCurrency = await getActivityCurrency(
    info.assetAddress,
    chainId,
    undefined,
    info.assetSymbol,
  ...
  const formattedShares = shareCurrency
    ? formatNumber({
        input: parseFloat(CurrencyAmount.fromRawAmount(shareCurrency, info.shareAmountRaw).toSignificant()),
        type: NumberType.TokenNonTx,
      })
    : i18n.t('common.unknown')
  ...
  return {
    title: getActivityTitle(info.type, status),
    descriptor: i18n.t('activity.transaction.lending.redeem.descriptor', {
      amountWithSymbol: `${formattedShares} ${i18n.t('common.shares')}`,
      vaultLabel: info.vaultTitle,
    }),
...
function parseMorphoAuthorization(
  info: MorphoAuthorizationTransactionInfo,
  status: TransactionStatus,
): Partial<Activity> {
  return {
    title: getActivityTitle(info.type, status),
    descriptor: i18n.t('activity.transaction.lending.authorization.descriptor', {
      label: info.protocolLabel,
    }),
  }
}
...
export async function transactionToActivity(
  ...
  try {
    ...
    if (info.type === TransactionType.SWAP) {
    ...
    } else if (info.type === TransactionType.MORPHO_AUTHORIZATION) {
      additionalFields = parseMorphoAuthorization(info, details.status)
    } else if (
      info.type === TransactionType.MORPHO_SUPPLY ||
      info.type === TransactionType.BORROW ||
      info.type === TransactionType.REPAY ||
      (info.type === TransactionType.WITHDRAW_LIQUIDITY_STAKING &&
        'protocol' in info &&
        info.protocol === 'morpho-market')
    ) {
      additionalFields = await parseMorphoMarket(
        info as
          | MorphoSupplyTransactionInfo
          | MorphoBorrowTransactionInfo
          | MorphoRepayTransactionInfo
      ...
      info.type === TransactionType.MORPHO_REDEEM ||
      (info.type === TransactionType.DEPOSIT_LIQUIDITY_STAKING &&
        'protocol' in info &&
        info.protocol === 'morpho-vault') ||
      (info.type === TransactionType.WITHDRAW_LIQUIDITY_STAKING &&
        'protocol' in info &&
        info.protocol === 'morpho-vault')
    ) {
      additionalFields =
        info.type === TransactionType.MORPHO_REDEEM
          ? await parseMorphoVaultRedeem(info, chainId, details.status, formatNumber)
          : await parseMorphoVault(
              info as MorphoVaultDepositTransactionInfo | MorphoVaultWithdrawTransactionInfo,
              chainId,
              details.status,
              formatNumber,
            )
    } else if (info.type === TransactionType.WRAP) {
      if (info.inputCurrencyId && info.inputCurrencyId) {
        additionalFields = await parseFewWrap(info, chainId, details.status, formatNumber)
      } else {
