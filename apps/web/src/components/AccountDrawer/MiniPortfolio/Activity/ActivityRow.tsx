...
export function ActivityRow({ activity }: { activity: Activity }) {
  ...
  const onRemoveExpired = useCallback(
    (e: any) => {
      e.stopPropagation()
      removeTransaction(hash)
    },
    [hash, removeTransaction],
  )
  ...
  return (
    ...
    >
      <PortfolioRow
        ...
        right={
          activity.status === TransactionStatus.Pending && activity.timestamp < Date.now() - 1000 * 60 * 10 ? (
            <TouchableArea onPress={onRemoveExpired} p="$spacing4">
              <Trash color="$neutral3" size="$icon.20" />
            </TouchableArea>
          ) : undefined
        }
        onClick={onClick}
