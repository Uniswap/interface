...
function mockMultiStatus(
  ...
  return [
    [
      {
        info,
        ...mockCommonFields(id, mockAccount2, TransactionStatus.Pending),
    ...
    [
      {
        info,
        ...mockCommonFields(id, mockAccount2, TransactionStatus.Confirmed),
      } as unknown as TransactionDetails,
      mockChainId,
    ],
    [
      {
        info,
        ...mockCommonFields(id, mockAccount2, TransactionStatus.Failed),
      } as unknown as TransactionDetails,
      mockChainId,
    ],
  ];
...
describe("parseLocalActivity", () => {
  ...
  it("only returns activity for the current account", async () => {
    const { result: result1, waitFor } = renderHook(() =>
      useLocalActivities(mockAccount1)
    );
    const { result: result2 } = renderHook(() =>
      useLocalActivities(mockAccount2)
    );
  ...
  it("Properly uses correct tense of activity title based on tx status", async () => {
    ...
    expect(
      result.current[mockHash("0xswap_exact_input", TransactionStatus.Pending)]
        ?.title
    ).toEqual("Swapping");
    expect(
      result.current[
        mockHash("0xswap_exact_input", TransactionStatus.Confirmed)
      ]?.title
    ).toEqual("Swapped");
    expect(
      result.current[mockHash("0xswap_exact_input", TransactionStatus.Failed)]
        ?.title
    ).toEqual("Swap failed");
  });
