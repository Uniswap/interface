...
export default function TableNetworkFilter({
  ...
  const orderedChainIds = useOrderedChainIds(chainIds)
  const hasCustomChainIds = chainIds !== ALL_CHAIN_IDS
  ...
  const tableNetworkItemRenderer = useCallback(
    (chainId: UniverseChainId) => {
      if (!hasCustomChainIds && !isSupportedChainCallback(chainId)) {
        return null
      }
    ...
    [hasCustomChainIds, isSupportedChainCallback, tab],
  )
