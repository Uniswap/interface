...
import { RingChartTimePeriod, TimePeriod, toRingHistoryDuration } from 'appGraphql/data/util'
...
import {
  useRingTDPPriceChartData,
  useRingTDPTVLChartData,
  useRingTDPVolumeChartData,
...
export const DEFAULT_CHART_PILL_TIME_SELECTOR_OPTIONS = RING_CHART_ORDERED_TIMES.map((time: RingChartTimePeriod) => ({
  value: RING_CHART_DISPLAYS[time],
})) as SegmentedControlOption[]
...
export function useCreateTDPChartState(
  ...
  currencyChainName: Chain,
): TDPRingChartState {
  const [timePeriod, setTimePeriod] = useState<RingChartTimePeriod>(RingChartTimePeriod.DAY)
  ...
  const variables = { address: tokenDBAddress, chain: currencyChainName, duration: toRingHistoryDuration(timePeriod) }
  ...
  const priceQuery = useRingTDPPriceChartData(variables, chartType !== ChartType.PRICE, priceChartType)
  const volumeQuery = useRingTDPVolumeChartData(variables, chartType !== ChartType.VOLUME)
  const tvlQuery = useRingTDPTVLChartData(variables, chartType !== ChartType.TVL)
    ...
    <ChartActionsContainer>
      ...
      <Flex $md={{ width: '100%' }}>
        <SegmentedControl
          ...
          selectedOption={RING_CHART_DISPLAYS[timePeriod]}
          onSelectOption={(option) => {
            const time = getTimePeriodFromDisplay(option as TimePeriodDisplay)
            if ((time as unknown as RingChartTimePeriod) === timePeriod) {
              refitChartContent?.()
            } else {
              setTimePeriod(time as unknown as RingChartTimePeriod)
            }
          }}
        />
      </Flex>
