...
export function checkDataQuality(
  data: { time: number }[],
  chartType: ChartType,
  duration: HistoryDuration | RingHistoryDuration,
): DataQuality {
  if (data.length < 3) {
...
export function withUTCTimestampPool<T extends { date: number }>(entry: T): T & { time: UTCTimestamp } {
  const value = Number((entry as any).volumeUSD)
    ? Number((entry as any).volumeUSD)
    : Number((entry as any).untrackedVolumeUSD)
  return {
    ...entry,
    timestamp: Number(entry.date) as UTCTimestamp,
    time: Number(entry.date) as UTCTimestamp,
    value,
    volumeUSD: value,
  }
}
