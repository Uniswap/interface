import { Currency } from '@ring-protocol/sdk-core'
...
export function TokenDescription() {
  ...
  const { address, token, tokenQuery, currencyChainId } = useTDPContext()
  ...
  const explorerUrl = getExplorerLink(
    currencyChainId,
    address,
    (currency as unknown as Currency)?.isNative ? ExplorerDataType.NATIVE : ExplorerDataType.TOKEN,
  )
  ...
  const { inputTax: sellFee, outputTax: buyFee } = useSwapTaxes(address, address, currencyChainId)
  ...
  return (
    <TokenInfoSection data-testid="token-details-info-section">
      ...
      <TokenButtonRow data-testid="token-details-info-links">
        {!(currency as unknown as Currency)?.isNative && (
          <MouseoverTooltip
            disabled
            placement="bottom"
          ...
          >
            <TokenInfoButton onPress={copy}>
              ...
              {shortenAddress(currency?.address ?? address)}
            </TokenInfoButton>
          </MouseoverTooltip>
        )}
