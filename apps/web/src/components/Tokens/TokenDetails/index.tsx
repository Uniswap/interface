...
function TDPBreadcrumb() {
  const { currencyChain, token } = useTDPContext()
  ...
  return (
    <BreadcrumbNavContainer aria-label="breadcrumb-nav">
      ...
      <CurrentPageBreadcrumb address={token?.address} currency={token} />
...
function TDPSwapComponent() {
  const { address, token, currencyChainId, tokenColor } = useTDPContext()
  const navigate = useNavigate()

  const ringTokens = useDefaultRingActiveTokens(currencyChainId)
  const tokenInfo = Object.values(ringTokens).find(
    (item) => item.address.toLowerCase() === address?.toLowerCase(),
  ) as any

  const backendToken = new Token(
    currencyChainId,
    address === NATIVE_CHAIN_ID ? DEFAULT_NATIVE_ADDRESS_LEGACY : address,
    tokenInfo?.decimals || 18,
    tokenInfo?.symbol || '',
    tokenInfo?.name || '',
  )
  const backendCurrency = unwrapToken(currencyChainId, backendToken)
  const unwrapCurrency = unwrapFewToken(currencyChainId, token)
  const currency = unwrapCurrency || backendCurrency

  const currencyInfo = useCurrencyInfo(currencyId(currency as unknown as Currency))
  ...
  const initialOutputCurrency = areCurrenciesEqual(initialInputCurrency, currency as unknown as Currency)
    ? outputCurrency
    : currency
  ...
  return (
    <Flex gap="$gap12">
      <Swap
        ...
        initialOutputCurrency={initialOutputCurrency as Currency}
...
export default function TokenDetails() {
  const { token, currencyChainId, address } = useTDPContext()
  const tokenQueryData = token
  const media = useMedia()
  const showRightPanel = !media.xl
  const { direction: scrollDirection } = useScroll()
  const isTouchDevice = useIsTouchDevice()
  const currency = unwrapFewToken(currencyChainId, token)
    ...
    <TDPAnalytics>
      <TokenDetailsLayout>
        <LeftPanel>
          ...
          <StatsSection chainId={currencyChainId} address={address} tokenQueryData={tokenQueryData} />
          <DividerLine />
          <ActivitySection />
        </LeftPanel>
        <RightPanel>
