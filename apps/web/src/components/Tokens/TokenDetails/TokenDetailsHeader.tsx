...
export const TokenDetailsHeader = () => {
  const { currency, tokenQuery, token, address } = useTDPContext()
  ...
  const ringTokens = useDefaultRingActiveTokens(currency.chainId)
  const tokenInfo = Object.values(ringTokens).find(
    (item) => item.address.toLowerCase() === address?.toLowerCase(),
  ) as any
  const originToken = unwrapFewToken(currency.chainId, token, tokenInfo?.logoURI)

  const tokenSymbolName = originToken?.symbol ?? tokenInfo?.symbol ?? t('tdp.symbolNotFound')

  const explorerUrl = getExplorerLink(
    currency.chainId,
    originToken?.address ?? (address as string),
    currency.isNative ? ExplorerDataType.NATIVE : ExplorerDataType.TOKEN,
  )

  const { homepageUrl, twitterName } = tokenQuery?.token?.project ?? {}
  ...
  const twitterShareName =
    token?.originToken?.name && token?.originToken?.symbol
      ? `${token?.originToken?.name} (${token?.originToken?.symbol})`
      : token?.originToken?.name || token?.originToken?.symbol || ''
  ...
  return (
    ...
    >
      <Flex row alignItems="center" $sm={{ alignItems: 'flex-start', flexDirection: 'column' }} gap="$gap12">
        <PortfolioLogo images={[tokenInfo?.logoURI]} chainId={currency.chainId} size={32} />
        <Flex row gap="$gap8" alignItems="center">
          <Text variant="heading3" minWidth={40} {...EllipsisTamaguiStyle}>
            {originToken?.name ?? tokenInfo?.name ?? t('tdp.nameNotFound')}
          </Text>
