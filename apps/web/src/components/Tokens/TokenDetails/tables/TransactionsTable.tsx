...
import { useRingTokenTransactions } from 'appGraphql/data/ring/useRingTokenTransactions'
...
export function TransactionsTable({ chainId, referenceToken }: { chainId: UniverseChainId; referenceToken: Token }) {
  ...
  const { transactions, error, loading, loadMore } = useRingTokenTransactions(referenceToken.address, chainId, [
  ...
  const unwrappedReferenceToken = unwrapFewToken(chainId, referenceToken)

  const data = useMemo(
    () =>
      transactions.map((transaction: any) => {
        let nonReferenceTokenInfo
        if (
          transaction.token0.originToken.address.toLowerCase() === referenceToken?.originToken?.address?.toLowerCase()
        ) {
          nonReferenceTokenInfo = Object.values(ringTokens).find(
            (item) => item.address.toLowerCase() === transaction.token1.originToken.address.toLowerCase(),
          ) as any
        } else {
          nonReferenceTokenInfo = Object.values(ringTokens).find(
            (item) => item.address.toLowerCase() === transaction.token0.originToken.address.toLowerCase(),
          ) as any
        }

        const swapLeg0 = {
          address: transaction.token0.originToken.address,
          symbol: transaction.token0.originToken.symbol,
          amount: parseFloat(transaction.token0Quantity),
          token: transaction.token0,
        }
        const swapLeg1 = {
          address: transaction.token1.originToken.address,
          symbol: transaction.token1.originToken.symbol,
        ...
        return {
          ...
          nonReferenceLogo: nonReferenceTokenInfo?.logoURI,
        }
      }),
    [transactions, referenceToken, ringTokens],
  )
  ...
  const columns = useMemo(() => {
      ...
      columnHelper.accessor((row) => row.output.address, {
        ...
        cell: (outputTokenAddress) => {
          const isBuy =
            ...
            referenceToken?.originToken?.address?.toLowerCase()
          return (
            <Cell loading={showLoadingSkeleton} justifyContent="flex-start" grow>
              <TableText color={isBuy ? '$statusCritical' : '$statusSuccess'}>
                {isBuy ? <Trans i18nKey="common.sell.label" /> : <Trans i18nKey="common.buy.label" />}
              </TableText>
            </Cell>
          )
        },
      ...
      columnHelper.accessor(
        (row) => {
          const nonReferenceSwapLeg =
            row.input.address?.toLowerCase() === referenceToken?.originToken?.address?.toLowerCase()
              ? row.output
              : row.input
          return (
            <Flex row gap="$gap8" justifyContent="flex-end" alignItems="center">
              ...
              <RingTokenLinkCell token={nonReferenceSwapLeg.token as unknown as Token} logo={row.nonReferenceLogo} />
            </Flex>
          )
        },
        {
    ...
    unwrappedReferenceToken.symbol,
    referenceToken?.originToken?.address,
    formatNumber,
    activeLocalCurrency,
    formatFiatPrice,
  ])
