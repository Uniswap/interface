...
export function getExploreProtocolVersionLabel(version: ProtocolVersion): string | undefined {
  switch (version) {
    case ProtocolVersion.V2:
      return 'v2'
    case ProtocolVersion.V3:
...
export function parseProtocolVersion(version: string | undefined): ProtocolVersion | undefined {
  switch (version?.toLowerCase()) {
    ...
    case 'fewtoken':
      return ProtocolVersion.Fewv2
    default: {
      logger.warn('Liquidity/utils.tsx', 'parseProtocolVersion', `Unknown protocol version: ${version ?? 'undefined'}`)
      return undefined
    }
  }
}
...
export function getPositionUrl(position: PositionInfo): string {
  ...
  if (position.version === ProtocolVersion.Fewv2) {
    return `/positions/fewv2/${chainUrlParam}/${position.liquidityToken.address}`
  } else if (position.version === ProtocolVersion.V2) {
...
export function parseRestPosition(position?: RestPosition): PositionInfo | undefined {
  try {
    ...
    const protocolVersion = position.protocolVersion
    const protocolVersionNumber = protocolVersion as unknown as number
    const protocolVersionString = String(protocolVersion)

    // Check if this is a pair position (v2Pair case)
    const positionCase = (position.position as any)?.case
    const isPairPosition = positionCase === 'v2Pair'
    const pairData = position.position?.value

    // Check if this is Fewv2 based on protocolVersion
    // API returns "PROTOCOL_VERSION_FEWV2" as string, or enum value 4
    const isFewv2 =
      protocolVersion === ProtocolVersion.Fewv2 ||
      protocolVersionNumber === 4 ||
      protocolVersionString === 'PROTOCOL_VERSION_FEWV2' ||
      protocolVersionString.includes('FEWV2') ||
      protocolVersionString.includes('Fewv2')

    if (isPairPosition && pairData) {
      const v2PairPosition = pairData as PairPosition
      const token0 = parseRestToken<Token>(v2PairPosition.token0)
      const originToken0 = parseRestToken<Token>(v2PairPosition.originToken0)
      const token1 = parseRestToken<Token>(v2PairPosition.token1)
      const originToken1 = parseRestToken<Token>(v2PairPosition.originToken1)
      const liquidityToken = parseRestToken<Token>(v2PairPosition.liquidityToken)
      if (!token0 || !originToken0 || !token1 || !originToken1 || !liquidityToken) {
        return undefined
      }

      // Check if this is Fewv2 based on token symbols (Fewv2 tokens start with "fw")
      // This is a fallback when protocolVersion is not correctly set
      const token0Symbol = token0.symbol?.toLowerCase() || ''
      const token1Symbol = token1.symbol?.toLowerCase() || ''
      const hasFewTokenSymbols = token0Symbol.startsWith('fw') && token1Symbol.startsWith('fw')

      // Final Fewv2 check: protocolVersion OR token symbols
      const finalIsFewv2 = isFewv2 || hasFewTokenSymbols

      if (finalIsFewv2) {
        // Handle Fewv2 pair
        const fewPair = getFewPairFromRest({ pair: v2PairPosition, token0, token1 })
        return {
          ...
          currency0Amount: CurrencyAmount.fromRawAmount(originToken0, v2PairPosition.liquidity0),
          currency1Amount: CurrencyAmount.fromRawAmount(originToken1, v2PairPosition.liquidity1),
          fewToken0Amount: CurrencyAmount.fromRawAmount(token0, v2PairPosition.liquidity0),
          fewToken1Amount: CurrencyAmount.fromRawAmount(token1, v2PairPosition.liquidity1),
