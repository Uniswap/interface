...
export function OutageBanner({ chainId, version }: ChainOutageData) {
  ...
  const chainName = capitalize(
    chainIdToBackendChain({ chainId, withFallback: true }).toLowerCase()
  );
      ...
      <Container>
        ...
        <OutageTextContainer>
          ...
          <ThemedText.LabelMicro>
            <Trans
              i18nKey="outageBanner.message"
              values={{ chainName, versionDescription }}
            />
          </ThemedText.LabelMicro>
          <ThemedText.LabelMicro>
            <Trans i18nKey="outageBanner.message.sub" />
          </ThemedText.LabelMicro>
          <HelpCenterLink href="http://docs.taraswap.info/docs/outage-and-bug-flags">
