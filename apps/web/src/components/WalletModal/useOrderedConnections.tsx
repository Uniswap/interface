...
function getInjectedConnectors(connectors: readonly Connector[], isEmbeddedWalletEnabled?: boolean) {
  ...
  const injectedConnectors = connectors.filter((c) => {
    ...
    // Special-case: Ignore the Ring Extension injection here if it's being displayed separately. This logic is updated with Embedded Wallet support where the Ring Extension is displayed with other connectors
...
/**
 * These connectors do not include Ring Wallets because those are
...
export function useOrderedConnections(options?: { showSecondaryConnectors?: boolean }): InjectableConnector[] {
  ...
  return useMemo(() => {
    ...
    // Hidden: Passkey/Embedded wallet option is disabled for Ringswap
    ...
    if (showSecondaryConnectors) {
      // Hidden: Passkey/Embedded wallet option is disabled for Ringswap
      // if (isMobileWeb && embeddedWalletConnector) {
