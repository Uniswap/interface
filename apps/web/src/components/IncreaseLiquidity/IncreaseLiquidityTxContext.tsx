...
function withFewTokenRouteVariant<T extends object>(
  request: T,
  shouldUseFewTokenRoute: boolean,
): T & { routeVariant?: 'FEWTOKEN' } {
  if (!shouldUseFewTokenRoute) {
    return request
  }
...
export function IncreaseLiquidityTxContextProvider({ children }: PropsWithChildren): JSX.Element {
  ...
  const shouldUseFewTokenRoute =
    positionInfo?.version === ProtocolVersion.Fewv2 &&
    Boolean(
      currencyAmounts?.TOKEN0?.currency.isToken &&
        isFewToken(currencyAmounts.TOKEN0.currency) &&
        currencyAmounts?.TOKEN1?.currency.isToken &&
        isFewToken(currencyAmounts.TOKEN1.currency),
    )
  ...
  const increaseLiquidityApprovalParams: CheckApprovalLPRequest | undefined = useMemo(() => {
    ...
    // Use currencyAmounts.TOKEN0/TOKEN1.currency instead of positionInfo.currency0Amount.currency
    // because currencyAmounts already accounts for unwrapNativeCurrency (if user selected "Add as ETH")
    // Native currencies don't need approval, so this ensures we don't request approval for native ETH
    const token0Currency = currencyAmounts.TOKEN0.currency
    const token1Currency = currencyAmounts.TOKEN1.currency
    const request = {
      simulateTransaction: true,
      walletAddress: account.address,
      chainId: positionInfo.currency0Amount.currency.chainId,
      protocol: getProtocolItems(positionInfo.version),
      token0: getCurrencyAddressForTradingApi(token0Currency),
      token1: getCurrencyAddressForTradingApi(token1Currency),
    ...
    return withFewTokenRouteVariant(request, shouldUseFewTokenRoute) as CheckApprovalLPRequest
  }, [positionInfo, account.address, currencyAmounts, generatePermitAsTransaction, shouldUseFewTokenRoute])
  ...
  const increaseCalldataQueryParams = useMemo((): IncreaseLPPositionRequest | undefined => {
    ...
    const request = {
      ...
      defaultDependentAmount:
        positionInfo.version === ProtocolVersion.V2 || positionInfo.version === ProtocolVersion.Fewv2
          ? dependentAmount
          : undefined,
      position: {
    ...
    return withFewTokenRouteVariant(request, shouldUseFewTokenRoute) as IncreaseLPPositionRequest
  }, [
    account,
    positionInfo,
    currencyAmounts,
