...
import { PoolTableTransaction, PoolTableTransactionType } from 'appGraphql/data/pools/usePoolTransactions'
import { useRingPoolTransactions } from 'appGraphql/data/ring/useRingPoolTransactions'
...
function comparePoolTokens(tokenA: PoolTableTransaction['pool']['token0'], tokenB?: Token) {
  if (tokenB?.address === NATIVE_CHAIN_ID) {
    const chainId = supportedChainIdFromGQLChain(tokenB.chain as any)
...
export function PoolDetailsTransactionsTable({
  ...
  const { transactions, loading, loadMore, error } = useRingPoolTransactions(poolAddress, chainId, filter, token0)
