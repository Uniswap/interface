...
import { PoolSortFields, RingPoolSortFields, TablePool, TableRingPool } from 'appGraphql/data/pools/useTopPools'
import {
  OrderDirection,
  gqlToCurrency,
...
import { useExploreContextRingTopPools } from 'state/explore/topPools'
...
function RingPoolDescription({
  token0,
  token1,
  chainId,
}: {
  token0?: RingToken & { logoUrl?: string; isNative?: boolean }
  token1?: RingToken & { logoUrl?: string; isNative?: boolean }
  chainId: UniverseChainId
}) {
  const currencies = [token0 ? token0 : undefined, token1 ? token1 : undefined]
  ...
  return (
    <Flex row gap="$gap8" alignItems="center" maxWidth="100%">
      <PortfolioLogo
        images={[
          (currencies[0]?.isNative ? nativeLogo : token0?.logoUrl) as string,
          (currencies[1]?.isNative ? nativeLogo : token1?.logoUrl) as string,
        ]}
        chainId={chainId}
        size={24}
      />
...
export const sortRingMethodAtom = atomWithReset<RingPoolSortFields>(RingPoolSortFields.TVL)
...
interface RingTopPoolTableProps {
  topPools?: RingPoolStat[]
  isLoading: boolean
  isError: boolean
}
...
export const ExploreTopPoolTable = memo(function ExploreTopPoolTable() {
  ...
  const sortRingMethod = useAtomValue(sortRingMethodAtom)
  ...
  const {
    topPools: ringTopPools,
    isLoading: ringIsLoading,
    isError: ringIsError,
  } = useExploreContextRingTopPools(
    {
      sortBy: sortRingMethod,
      sortDirection: sortAscending ? OrderDirection.Asc : OrderDirection.Desc,
    },
    selectedProtocol,
  )

  return <TopPoolTable topPoolData={{ topPools: ringTopPools, isLoading: ringIsLoading, isError: ringIsError }} />
})
...
const TopPoolTable = memo(function TopPoolTable({
  ...
  topPoolData: RingTopPoolTableProps
  pageSize?: number
  staticSize?: boolean
...
export function RingPoolsTable({
  pools,
  loading,
  error,
  loadMore,
  ...
  pools?: TableRingPool[] | RingPoolStat[]
  loading: boolean
  error?: ApolloError | boolean
  loadMore?: ({ onComplete }: { onComplete?: () => void }) => void
  maxWidth?: number
  ...
  const poolTableValues: PoolTableValues[] | undefined = useMemo(
    () =>
      pools?.map((pool, index) => {
        ...
        const poolIdentifier = (isV4Pool ? pool.poolId : pool?.address)?.toLowerCase()
        ...
        return {
          ...
          poolDescription: (
            <RingPoolDescription
              token0={unwrapFewToken(chainId, pool.token0, token0Info?.logoURI)}
              token1={unwrapFewToken(chainId, pool.token1, token1Info?.logoURI)}
              chainId={chainId}
            />
          ),
          protocolVersion: pool.protocolVersion?.toLowerCase(),
          feeTier: pool.feeTier,
          tvl: isGqlPool ? pool.tvl : giveExploreStatDefaultValue(Number(pool.totalValueLockedUSD)),
          volume24h: isGqlPool ? pool.volume24h : giveExploreStatDefaultValue(pool.volume1Day),
          volume30d: isGqlPool ? pool.volume30d : giveExploreStatDefaultValue(pool.volume30Day),
          volOverTvl: pool.volOverTvl,
          apr: pool.apr,
          // rewardApr: pool.boostedApr,
          link: `/explore/pools/${getChainUrlParam(chainId ?? defaultChainId)}/${isV4Pool ? pool.poolId : pool?.address}`,
          token0CurrencyId: currency0Id,
          token1CurrencyId: currency1Id,
          isWhitelisted,
          analytics: {
    ...
    [defaultChainId, filterString, pools, ringTokens, whitelistData],
  )
  ...
  const columns = useMemo(() => {
    ...
    const filteredColumns = [
      ...
      (whitelistData?.pools.length ?? 0) > 0
        ? columnHelper.accessor((row) => row.isWhitelisted, {
            id: 'referralReward',
            size: 40,
            header: () => <HeaderCell />,
            cell: (isWhitelisted) => (
              <Cell justifyContent="center" loading={false}>
                {isWhitelisted.getValue?.() ? (
