...
describe('trade price', () => {
  it('correctly renders the trade price', () => {
    ...
    const tradePriceToggle = screen.getByTestId(TestID.TradePriceToggle)
    expect(screen.getByText('1 USDC = 0.02 WETH')).toBeInTheDocument()
  ...
  it('handles zeroed numerator', () => {
    ...
    const tradePriceToggle = screen.getByTestId(TestID.TradePriceToggle)
    expect(screen.getByText('1 USDC = 0 WETH')).toBeInTheDocument()
    expect(tradePriceToggle).toBeInTheDocument()
    fireEvent.click(tradePriceToggle)
