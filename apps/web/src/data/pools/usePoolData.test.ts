...
// `feeTier`, not the SDK's dynamic-fee sentinel — regression coverage for the "Invariant failed"
// crash this caused when fed straight into a V4Pool construction.
const mockV4DynamicFeePoolData = {
  v4Pool: {
    ...mockV4PoolData.v4Pool,
    poolId: '0xv4dynamicpool123',
    feeTier: 1_000_000,
    isDynamicFee: true,
  },
}
...
describe('usePoolData', () => {
  ...
  it('should normalize a dynamic-fee v4 pool feeAmount to the SDK sentinel', () => {
    mockV4Query.mockReturnValue({
      loading: false,
      error: undefined,
      data: mockV4DynamicFeePoolData,
    })

    const { result } = renderHook(() =>
      usePoolData({
        poolIdOrAddress: '0xv4dynamicpool123',
        chainId: UniverseChainId.Mainnet,
        isPoolAddress: false,
      }),
    )

    expect(result.current.data?.feeTier?.feeAmount).toBe(DYNAMIC_FEE_AMOUNT)
    expect(result.current.data?.feeTier?.isDynamic).toBe(true)
  })
