...
export function usePoolData({
  ...
  return useMemo(() => {
    ...
    const isDynamicFee = dataV4?.v4Pool?.isDynamicFee ?? false
    const feeTier: FeeData = {
      ...
      // construction doesn't throw its "Invariant failed" fee check.
      feeAmount: isDynamicFee
        ? DYNAMIC_FEE_AMOUNT
        : (dataV4?.v4Pool?.feeTier ?? dataV3?.v3Pool?.feeTier ?? V2_DEFAULT_FEE_TIER),
