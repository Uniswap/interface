...
export const ALL_TX_DEFAULT_QUERY_SIZE = 20

export interface UseAllTransactionsResult {
  transactions: GraphQLApi.PoolTxFragment[]
  loading: boolean
  errorV2?: ApolloError
  errorV3?: ApolloError
  errorV4?: ApolloError
...
export function useAllTransactionsLegacy({
  chain,
  filter,
  skip,
}: {
  ...
  const skipTransactionsQueries = skip || !isWindowVisible || fromGraphQLChain(chain) === UniverseChainId.Solana
