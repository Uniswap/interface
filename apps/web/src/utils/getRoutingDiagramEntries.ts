    ...
    for (let i = 0; i < pools.length; i++) {
      ...
      const poolProtocol =
        nextPool instanceof Pair
          ? Protocol.V2
          : nextPool instanceof FewPair
            ? Protocol.FEWV2
            : nextPool instanceof V3Pool
              ? Protocol.V3
              : Protocol.V4
      const feeTier =
        nextPool instanceof Pair || nextPool instanceof FewPair
          ? V2_DEFAULT_FEE_TIER
          : 'fee' in nextPool
            ? (nextPool as any).fee
            : V2_DEFAULT_FEE_TIER
      const entry: RoutingDiagramEntry['path'][0] = [tokenIn, tokenOut, feeTier, poolProtocol]
      path.push(entry)
    }
    return {
      percent,
