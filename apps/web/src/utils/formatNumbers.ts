...
function formatNumber({
  ...
  // For compact notation, use en-US to get K/M/B/T suffixes (StatCard only supports these)
  // but preserve currency symbol from original locale
  const useCompactNotation = formatterOptions.notation === 'compact'
  const formatLocale = useCompactNotation && locale !== DEFAULT_LOCALE ? DEFAULT_LOCALE : locale

  const formatValue = (value: number): string => {
    if (useCompactNotation && formatLocale !== locale && formatterOptions.currency) {
      // Get K/M/B/T from en-US, currency symbol from original locale
      const enParts = new Intl.NumberFormat(formatLocale, formatterOptions).formatToParts(value)
      const originalParts = new Intl.NumberFormat(locale, {
        style: 'currency',
        currency: localCurrency,
        currencyDisplay: formatterOptions.currencyDisplay,
      }).formatToParts(value)
      const currencySymbol = originalParts.find((part) => part.type === 'currency')?.value
      const isSymbolBeforeNumber = originalParts[0]?.type === 'currency'

      if (currencySymbol) {
        // Extract number + compact suffix (K/M/B/T) from en-US parts
        ...
        const numberWithSuffix = numberParts.join('')
        return isSymbolBeforeNumber ? `${currencySymbol}${numberWithSuffix}` : `${numberWithSuffix}${currencySymbol}`
      }
    }
    return new Intl.NumberFormat(formatLocale, formatterOptions).format(value)
  }
  ...
  return (prefix ?? '') + formatValue(hardCodedInputValue)
}
