...
} from '~/features/Toucan/Auction/BidDistributionChart/markers/markerPositions'
import { BidGroup, MarkerPosition } from '~/features/Toucan/Auction/BidDistributionChart/markers/types'
      ...
      if (clearingPriceBigInt !== null && group.bids.length > 0) {
        try {
          for (const bid of group.bids) {
            bidRangeMap[bid.bidId] = BigInt(bid.maxPrice) >= clearingPriceBigInt
          }
        } catch {
