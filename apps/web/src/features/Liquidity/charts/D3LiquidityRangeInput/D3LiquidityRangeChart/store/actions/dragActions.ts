...
export function createDragActions(get: () => ChartStoreState) {
  return {
    createHandleDragBehavior: (lineType: 'min' | 'max') => {
      ...
      // The dragged handle is rendered at tickToY(tick, alignment) — 'bottom' for
      // min, 'top' for max. 'top' adds tickSpacing, so converting a dragged Y back
      // to a tick must subtract tickSpacing for the max handle to keep the
      // round-trip invertible.
      const otherAlignment = lineType === 'min' ? 'top' : 'bottom'
      const draggedHandleOffset = lineType === 'min' ? 0 : tickSpacing
      const yToHandleTick = (y: number) => yToTick(y) - draggedHandleOffset
      ...
      const calculateFinalTicks = (draggedY: number) => {
        ...
        // Get raw tick from Y (alignment-corrected for max) and snap to tickSpacing
        const snappedTick = snapTickToSpacing(yToHandleTick(draggedY), tickSpacing)
        ...
        const otherY = tickToY({ tick: otherTick, tickAlignment: otherAlignment })

        const shouldSwap = shouldSwapLines(snappedTick, otherTick)
        const constrainedY = applyMinHeightConstraint(draggedY, otherY)
        const constrainedTick = snapTickToSpacing(yToHandleTick(constrainedY), tickSpacing)
        ...
        if (shouldSwap) {
          ...
          // Check if swapped distance meets minimum (compare rendered handle Ys)
          const minHandleY = tickToY({ tick: finalMinTick, tickAlignment: 'bottom' })
          const maxHandleY = tickToY({ tick: finalMaxTick, tickAlignment: 'top' })
          const swappedDistance = Math.abs(maxHandleY - minHandleY)
          if (swappedDistance < CHART_DIMENSIONS.RANGE_INDICATOR_MIN_HEIGHT) {
            const constrainedSwapY =
              lineType === 'min'
                ? otherY - CHART_DIMENSIONS.RANGE_INDICATOR_MIN_HEIGHT
                : otherY + CHART_DIMENSIONS.RANGE_INDICATOR_MIN_HEIGHT
            const constrainedSwapTick = snapTickToSpacing(yToHandleTick(constrainedSwapY), tickSpacing)
    ...
    createTickBasedDragBehavior: () => {
      ...
      return d3
        ...
        .on('start', (event) => {
          ...
          // Store the initial offset relative to the range center, using rendered
          // handle Ys (top of max bucket, bottom of min bucket) so the center
          // matches the visible range indicator.
          const minHandleY = tickToY({ tick: minTick, tickAlignment: 'bottom' })
          const maxHandleY = tickToY({ tick: maxTick, tickAlignment: 'top' })
          const currentRangeCenterY = (maxHandleY + minHandleY) / 2
