...
function clampToTickRange(tick: number): number {
  return Math.max(TickMath.MIN_TICK, Math.min(TickMath.MAX_TICK, tick))
}

/**
 * Snaps a tick to the closest spacing boundary, clamped to the valid tick range.
 */
export function snapTickToSpacing(tick: number, tickSpacing: number): number {
  // nearestUsableTick requires integers
  return nearestUsableTick(Math.round(clampToTickRange(tick)), tickSpacing)
}

/**
 * Rounds a tick down to the lower edge of its containing spacing bucket,
 * clamped to the valid tick range.
 */
export function floorTickToSpacing(tick: number, tickSpacing: number): number {
  return Math.floor(clampToTickRange(tick) / tickSpacing) * tickSpacing
}
