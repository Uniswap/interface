...
export const PERSIST_VERSION = 63
...
export function customCreateMigrate(migrations: MigrationManifest, options: MigrationConfig): PersistMigrate {
  ...
  return async (state: PersistedState, currentVersion: number) => {
    try {
      ...
      logger.error(new Error('Migration failed', { cause: error }), {
        tags: { file: 'migrations.ts', function: 'customCreateMigrate' },
      })
      // Return undefined to initialize with default state
      return undefined
    }
  }
}
