...
describe('migration to v16', () => {
  ...
  it('should handle missing user object gracefully', () => {
    const stateWithoutUser: PersistAppStateV16 = {
      _persist: {
        version: 15,
        rehydrated: true,
      },
    ...
    const result = migration16(stateWithoutUser)

    expect(result?.tokens).toEqual({ dismissedTokenWarnings: {} })
    expect(result?._persist.version).toEqual(16)
  })
})
