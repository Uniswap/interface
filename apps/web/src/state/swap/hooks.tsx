...
export function useDerivedSwapInfo(state: SwapState): SwapInfo {
  ...
  const insufficientGas =
    ...
    balance?.lessThan(trade.trade.gasUseEstimate ?? 0);
...
export function queryParametersToCurrencyState(
  ...
  const inputCurrency = parseCurrencyFromURLParameter(
...
export function useInitialCurrencyState(): {
  ...
  const { initialInputCurrencyAddress, chainId } = useMemo(() => {
    ...
    return {
      initialInputCurrencyAddress: highestBalanceNativeTokenAddress,
      chainId: highestBalanceChainId,
    };
  }, [
    account.isConnected,
    balances,
