...
describe('Transactions hooks', () => {
  beforeEach(() => {
    ...
    jest.useFakeTimers()
    store.dispatch(clearAllTransactions({ chainId: ChainId.X1 }))
  })
  ...
  it('useTransactionAdder adds a transaction', () => {
    ...
    expect(store.getState().transactions[ChainId.X1][pendingTransactionResponse.hash]).toEqual({
      hash: pendingTransactionResponse.hash,
  ...
  it('useTransactionRemover removes a transaction', () => {
    ...
    expect(store.getState().transactions[ChainId.X1][pendingTransactionResponse.hash]).toBeUndefined()
  })

  describe('useHasPendingApproval', () => {
    it('returns true when there is a pending transaction', () => {
      addPendingTransaction(mockApprovalTransactionInfo)
      const { result } = renderHook(() => useHasPendingApproval(USDC, PERMIT2_ADDRESS))
  ...
  describe('useHasPendingRevocation', () => {
    it('returns true when there is a pending revocation', () => {
      addPendingTransaction(mockRevocationTransactionInfo)
      const { result } = renderHook(() => useHasPendingRevocation(USDC, PERMIT2_ADDRESS))
      expect(result.current).toBe(true)
    })
  ...
  describe('useTransactionCanceller', () => {
    it('Replaces the original tx with a cancel tx with a different hash', () => {
      ...
      const originalTransactionDetails = store.getState().transactions[ChainId.X1][pendingTransactionResponse.hash]

      act(() => canceller.current(pendingTransactionResponse.hash, ChainId.X1, '0x456'))

      expect(store.getState().transactions[ChainId.X1][pendingTransactionResponse.hash]).toBeUndefined()

      expect(store.getState().transactions[ChainId.X1]['0x456']).toEqual({
