...
describe('transaction reducer', () => {
  ...
  describe('finalizeTransaction', () => {
    ...
    it('sets receipt', () => {
      ...
      const tx = store.getState()[ChainId.X1]?.['0x0']
  ...
  describe('clearAllTransactions', () => {
    it('removes all transactions for the chain', () => {
      ...
      expect(Object.keys(store.getState())).toEqual([String(ChainId.X1), String(ChainId.OPTIMISM)])
      expect(Object.keys(store.getState()[ChainId.X1] ?? {})).toEqual(['0x0'])
      expect(Object.keys(store.getState()[ChainId.OPTIMISM] ?? {})).toEqual(['0x1'])
      store.dispatch(clearAllTransactions({ chainId: ChainId.X1 }))
      expect(Object.keys(store.getState())).toHaveLength(2)
      expect(Object.keys(store.getState())).toEqual([String(ChainId.X1), String(ChainId.OPTIMISM)])
      expect(Object.keys(store.getState()[ChainId.X1] ?? {})).toEqual([])
      expect(Object.keys(store.getState()[ChainId.OPTIMISM] ?? {})).toEqual(['0x1'])
    })
  })
  ...
  describe('cancelTransaction', () => {
    it('replaces original tx with a cancel tx', () => {
      ...
      const originalTx = store.getState()[ChainId.X1]?.['0x0']
      store.dispatch(
        cancelTransaction({
          chainId: ChainId.X1,
          hash: '0x0',
      ...
      expect(Object.keys(store.getState())).toEqual([String(ChainId.X1)])
      expect(Object.keys(store.getState()[ChainId.X1] ?? {})).toEqual(['0x1'])

      const cancelTx = store.getState()[ChainId.X1]?.['0x1']
