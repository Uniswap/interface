...
const TraitItem = ({
  ...
  const toggleShowFullTraitName = useCollectionFilters(
    (state) => state.toggleShowFullTraitName
  );

  const { shouldShow, trait_value, trait_type } = useCollectionFilters(
    (state) => state.showFullTraitName
  );
    ...
    function TraitRow({ data, index, style }: TraitRowProps) {
      ...
      const isTraitSelected = selectedTraits.find(
        ({ trait_type, trait_value }) =>
          trait_type === trait.trait_type &&
          String(trait_value) === String(trait.trait_value)
      );
      return (
        <TraitItem
          style={style}
  ...
  return traits.length ? (
    <TraitsHeader index={index} numTraits={traits.length} title={type}>
      ...
      <Column
        ...
        style={{
          height: `${Math.min(
            TRAIT_ROW_HEIGHT * searchedTraits.length,
            styles.MAX_FILTER_DROPDOWN_HEIGHT
          )}px`,
        }}
      >
