...
export const generateTweetForAsset = (asset: GenieAsset): string => {
  ...
  }%20(${asset.collectionName})%20https://app.rigoblock.com/nfts/asset/${asset.address}/${
    asset.tokenId
  }%20via%20@uniswap`
}
...
export const generateTweetForPurchase = (assets: UpdatedGenieAsset[], txHashUrl: string): string => {
  ...
  } with @RigoBlock 🦄\n\nhttps://app.rigoblock.com/nfts/${collectionUrl}\n${txHashUrl}`
  ...
  const tweetText =
    assets.length == 1
      ? `I just listed ${
          ...
          .join(', ')}. Buy it on @RigoBlock at https://app.rigoblock.com/${getAssetHref(assets[0])}`
      : `I just listed ${
          assets.length
        } items on @RigoBlock at https://app.rigoblock.com/nfts/profile\n\nCollections: ${mapAssetsToCollections(assets)
