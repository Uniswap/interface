...
export const LARA_TARAXA = new Token(
  ChainId.TARAXA,
...
export const STTARA_TARAXA = new Token(
  ChainId.TARAXA,
  "0x37Df886BE517F9c75b27Cb70dac0D61432C92FBE",
  18,
  "stTARA",
  "Staked Taraxa"
);
...
export const UNI: { [chainId: number]: Token } = {
  ...
  [ChainId.OPTIMISM]: new Token(
    ChainId.OPTIMISM,
    UNI_ADDRESSES[ChainId.OPTIMISM],
    18,
    "UNI",
    "Uniswap"
  ),
...
export function isTaraxa(
  chainId: number
): chainId is ChainId.TARAXA | ChainId.TARAXA_TESTNET {
  return chainId === ChainId.TARAXA || chainId === ChainId.TARAXA_TESTNET;
}

export function isTaraxaTestnet(
  chainId: number
): chainId is ChainId.TARAXA_TESTNET {
  return chainId === ChainId.TARAXA_TESTNET;
}
...
class TaraxaTestnetNativeCurrency extends NativeCurrency {
  equals(other: Currency): boolean {
    return other.isNative && other.chainId === this.chainId;
  }

  get wrapped(): Token {
    if (!isTaraxaTestnet(this.chainId)) {
      throw new Error("Not Taraxa");
    }
    const wrapped = WRAPPED_NATIVE_CURRENCY[this.chainId];
    invariant(wrapped instanceof Token);
  ...
  public constructor(chainId: number) {
    if (!isTaraxaTestnet(chainId)) {
      throw new Error("Not Taraxa Testnet");
    }
    super(chainId, 18, "TARA", "Tara");
  }
}
...
export function nativeOnChain(chainId: number): NativeCurrency | Token {
  ...
  } else if (isTaraxaTestnet(chainId)) {
    nativeCurrency = new TaraxaTestnetNativeCurrency(chainId);
  } else {
