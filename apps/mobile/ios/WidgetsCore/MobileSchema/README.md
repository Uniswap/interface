...
Locally, they run automatically as part of the WidgetsCore `[GraphQL] Apollo Generate Swift` build phase on every build.

In CI, they are generated once up front by the `🍏 Generate Apollo GraphQL Swift sources` step in `.github/actions/mobile_build/ios_environment_setup/action.yml` (right after `pod install`, before `xcodebuild`). That step also exports `SKIP_XCODE_APOLLO_CODEGEN=true`, which makes the in-Xcode build phase no-op for the rest of the job — otherwise it would wipe and regenerate the files mid-build and race the WidgetsCore compile (`Build input file cannot be found: …graphql.swift`).
