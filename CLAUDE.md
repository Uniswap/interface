...
- **Gap 3 — JSBI vs native-bigint skew (NOT fixed; systemic).** Route now assembles a v2 Trade → `Trade.maximumAmountIn` → `Percent.lessThan(ZERO)` → vendored sdk-core `tryParseFraction` throws "Could not parse fraction". Root cause: **`vendor/sdk-core` (labeled 7.17.0) is JSBI-based** (`tryParseFraction` only accepts `instanceof JSBI`/number/string), but **v2-sdk 4.21 / universal-router-sdk are native-bigint** and pass a native `bigint`. All SDKs share the one vendored sdk-core (no dup), so it's not a version dup — it's a numeric-impl mismatch. Fix requires rebuilding the vendored sdk-core fork on a **native-bigint** base (real npm sdk-core 7.x + HookSwap chains re-added) — a coordinated, well-tested fork change, NOT a hot patch. This blocks quote+execution on the custom chains until aligned.
- **Net:** routing discovery + gas are fixed & live; the bigint alignment + real USD-pool liquidity are the remaining routing work. Both SOR fixes are committed/pushed/deployed; adapter healthy in embed mode.
