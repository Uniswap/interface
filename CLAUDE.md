...
- **Still needs Reggie (unchanged sole blocker + 1 creds item):** (1) **seed on-chain liquidity** (`contracts/seed/*.s.sol`, funded key) — until then every quote 404s; (2) enable **Sepolia on the Infura project** (or keep publicnode) if Infura-served Sepolia is wanted; (3) rebuild+redeploy the frontend SPA to ship the rebrand-image repoints.
