...
describe('time-to-swap logging', () => {
  it('completes two swaps and verifies the TTS logging for the first', () => {
    ...
    cy.waitForAmplitudeEvent(SwapEventName.SWAP_TRANSACTION_COMPLETED).then((event: any) => {
      ...
      cy.wrap(event.event_properties).should('have.property', 'time_to_sign')
      cy.wrap(event.event_properties.time_to_sign).should('be.a', 'number')
      cy.wrap(event.event_properties.time_to_sign).should('be.gte', 0)
    })
    ...
    cy.waitForAmplitudeEvent(SwapEventName.SWAP_TRANSACTION_COMPLETED).then((event: any) => {
      ...
      cy.wrap(event.event_properties).should('not.have.property', 'time_to_sign')
    })
  })
})
