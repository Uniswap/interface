...
beforeEach(() => {
  ...
  cy.intercept('https://api.uniswap.org/v1/quote', (req) => {
    req.headers['origin'] = 'https://app.uniswap.org'
    req.continue()
  })

  cy.intercept('https://beta.api.uniswap.org/v1/quote', (req) => {
    req.headers['origin'] = 'https://app.uniswap.org'
    req.continue()
  })
  // Graphql security policies are based on Origin headers.
  // These are stripped by cypreqs because chromeWebSecurity === false; this adds them back in.
  cy.intercept('https://api.uniswap.org/v1/graphql', (req) => {
    req.headers['origin'] = 'https://app.uniswap.org'
    req.continue()
  })
  cy.intercept('https://beta.api.uniswap.org/v1/graphql', (req) => {
    req.headers['origin'] = 'https://app.uniswap.org'
    req.continue()
  })
