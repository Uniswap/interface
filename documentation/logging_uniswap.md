...
![Token Purchase](./images/uniswap_token_purchase.png)
...
![Exchange logging events](./images/uniswap_exchange_events.png)

## Web3.py - (see above for Web3.js)
...
```
```python
deployedContract = web3.eth.contract(abi=uniswapExchangeAbi,address=Web3.toChecksumAddress("0xaf1a51fdca46190e7703b6cf97470efc92ec6498"))
```
### Get all entries
```
# Get all entries
# Create a filter
eventTokenPurchase = deployedContract.events.TokenPurchase.createFilter(fromBlock=1, toBlock='latest')
# Get entries
allEntries = eventTokenPurchase.get_all_entries()
```
### Get new entries
```python
# Create a filter
eventTokenPurchase = deployedContract.events.TokenPurchase.createFilter(fromBlock='latest')
# Check filter is live
eventTokenPurchase.stopped
# False
newEntries = eventTokenPurchase.get_new_entries()
```
### Listening for events synchronously
Please perform "Setting up the environment" before proceeding with the following code (I did not repeat the set up because the abi text takes up too much room on this page"
```python
...
eventTokenPurchase = deployedContract.events.TokenPurchase.createFilter()

log_loop(eventTokenPurchase, 2)
```
