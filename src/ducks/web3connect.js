...
export const watchBalance = ({ balanceOf, tokenAddress }) => (
  ...
  if (!tokenAddress) {
    ...
    if (
      watched.balances[tokenAddress] &&
      watched.balances[tokenAddress].includes(balanceOf)
    ) {
      return
    }
    dispatch({
...
export const watchApprovals = ({ tokenAddress, tokenOwner, spender }) => (
  dispatch,
  getState
) => {
  const {
...
export const sync = () => async (dispatch, getState) => {
  ...
  Object.keys(watched.balances).forEach(tokenAddress => {
    ...
    let contract =
      contracts[tokenAddress] || new web3.eth.Contract(ERC20_ABI, tokenAddress)
    let contractBytes32 =
      contracts[tokenAddress] ||
      new web3.eth.Contract(ERC20_WITH_BYTES_ABI, tokenAddress)
    ...
    watchlist.forEach(async address => {
      ...
      const decimals =
        tokenBalance.decimals || (await contract.methods.decimals().call())
      ...
      if (
        tokenBalance.value.isEqualTo(BN(balance)) &&
        tokenBalance.label &&
  ...
  Object.entries(watched.approvals).forEach(([tokenAddress, token]) => {
    const contract =
      contracts[tokenAddress] || new web3.eth.Contract(ERC20_ABI, tokenAddress)
    const contractBytes32 =
      contracts[tokenAddress] ||
      new web3.eth.Contract(ERC20_WITH_BYTES_ABI, tokenAddress)

    Object.entries(token).forEach(([tokenOwnerAddress, tokenOwner]) => {
      tokenOwner.forEach(async spenderAddress => {
        const approvalBalance = getApprovals(
          tokenAddress,
          tokenOwnerAddress,
          spenderAddress
        )
        const balance = await contract.methods
          .allowance(tokenOwnerAddress, spenderAddress)
          .call()
        const decimals =
          approvalBalance.decimals || (await contract.methods.decimals().call())
        ...
        try {
          ...
          try {
            symbol =
              symbol ||
              web3.utils.hexToString(
                await contractBytes32.methods.symbol().call()
              )
          } catch (err) {}
        }

        if (
          approvalBalance.label &&
          approvalBalance.value.isEqualTo(BN(balance))
        ) {
          return
        }
...
export default function web3connectReducer(
  state = initialState,
  { type, payload }
) {
  switch (type) {
