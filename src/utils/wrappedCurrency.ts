...
import { WEVMOS, EVMOS } from 'constants/tokens'
...
export function unwrappedToken(token: Token): Currency {
  ...
  if (formattedChainId && token.equals(WEVMOS[formattedChainId])) return EVMOS
  return token
}
...
function wrappedCurrencyInternal(currency: Currency, chainId: ChainId): Token {
  ...
  if (currency.isNative) return WEVMOS[chainId]
