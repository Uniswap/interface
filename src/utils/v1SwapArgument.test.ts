...
describe('v1SwapArguments', () => {
  ...
  it('exact token to eth', () => {
    const trade = Trade.exactIn(new Route([USDC_WETH], constants[ChainId.MAINNET].tokens.USDC, ETHER), new TokenAmount(constants[ChainId.MAINNET].tokens.USDC, '100'))
    const result = v1SwapArguments(trade, {
      recipient: TEST_RECIPIENT_ADDRESS,
      allowedSlippage: new Percent('1', '100'),
      deadline: 40 * 60
  ...
  it('exact token to token', () => {
    const trade = Trade.exactIn(new Route([USDC_WETH, DAI_WETH], constants[ChainId.MAINNET].tokens.USDC), new TokenAmount(constants[ChainId.MAINNET].tokens.USDC, '100'))
    const result = v1SwapArguments(trade, {
      recipient: TEST_RECIPIENT_ADDRESS,
      allowedSlippage: new Percent('1', '100'),
      deadline: 20 * 60
    ...
    expect(result.args[5]).toEqual(constants[ChainId.MAINNET].tokens.DAI.address)
    expect(result.value).toEqual('0x0')
  })
  it('eth to exact token', () => {
    const trade = Trade.exactOut(new Route([USDC_WETH], ETHER), new TokenAmount(constants[ChainId.MAINNET].tokens.USDC, '100'))
    const result = v1SwapArguments(trade, {
      recipient: TEST_RECIPIENT_ADDRESS,
      allowedSlippage: new Percent('1', '100'),
      deadline: 20 * 60
  ...
  it('token to exact token', () => {
    const trade = Trade.exactOut(new Route([USDC_WETH, DAI_WETH], constants[ChainId.MAINNET].tokens.USDC), new TokenAmount(constants[ChainId.MAINNET].tokens.DAI, '100'))
    const result = v1SwapArguments(trade, {
      recipient: TEST_RECIPIENT_ADDRESS,
      allowedSlippage: new Percent('1', '100'),
      deadline: 20 * 60
