...
import { NftCollection, useCollectionQuery, useRecentlySearchedCollectionQuery } from '../__generated__/types-and-hooks'
...
export function useRecentlySearchedCollections(addresses: string[]): {
  data: GenieCollection[] | undefined
  loading: boolean
} {
  const { data: queryData, loading } = useRecentlySearchedCollectionQuery({
    variables: {
      addresses,
    },
  })
  ...
  return { data: collections, loading }
}
