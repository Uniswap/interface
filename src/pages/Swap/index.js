...
import {
    getBlockDeadline
} from '../../helpers/web3-utils';
import {
    retry
} from '../../helpers/promise-utils';
...
class Swap extends Component {
    ...
    validate() {
        ...
        } = selectors().getBalance(account, inputCurrency);
    ...
    recalcTokenTokenForm = () => {
        ...
        const {
            value: inputReserveA,
            decimals: inputDecimalsA
        } = selectors().getBalance(exchangeAddressA, inputCurrency);
        const {
            value: outputReserveA
        } = selectors().getBalance(exchangeAddressA, 'OETH');
        const {
            value: inputReserveB
        } = selectors().getBalance(exchangeAddressB, 'OETH');
        const {
            value: outputReserveB,
    ...
    recalcEthTokenForm = () => {
        ...
        const {
            value: inputReserve,
            decimals: inputDecimals
        } = selectors().getBalance(exchangeAddress, inputCurrency);
        const {
            value: outputReserve,
        ...
        if (lastEditedField === INPUT) {
            ...
            const outputAmount = calculateEtherTokenOutput({
                inputAmount,
                inputReserve,
                outputReserve
            });
            ...
            const inputAmount = calculateEtherTokenInput({
                outputAmount,
                inputReserve,
                outputReserve
            });
    ...
    onSwap = async () => {
        ...
        const {
            decimals: outputDecimals
        } = selectors().getBalance(account, outputCurrency);
        let deadline;
        try {
        ...
        if (lastEditedField === INPUT) {
            ...
            switch (type) {
                case 'ETH_TO_TOKEN':
                    // update method so that we can calculate gas limit dynamically
                    web3.eth.getGasPrice()
                        .then(function(gasPrice) {
                            var contract = new web3.eth.Contract(EXCHANGE_ABI, fromToken[outputCurrency]);
                            contract.methods.ethToTokenSwapInput(BN(outputValue).multipliedBy(10 ** outputDecimals).multipliedBy(1 - ALLOWED_SLIPPAGE).toFixed(0), deadline, ).estimateGas({
                                    from: account
                                })
                ...
                case 'TOKEN_TO_ETH':
                    web3.eth.getGasPrice()
                        .then(function(gasPrice) {
                            var contract = new web3.eth.Contract(EXCHANGE_ABI, fromToken[inputCurrency]);
                            contract.methods
                                .tokenToEthSwapInput(
            ...
            switch (type) {
                ...
                case 'TOKEN_TO_ETH':
                    web3.eth.getGasPrice()
                        .then(function(gasPrice) {
                            var contract = new web3.eth.Contract(EXCHANGE_ABI, fromToken[inputCurrency]);
                            contract.methods.tokenToEthSwapOutput(
                                    BN(outputValue).multipliedBy(10 ** outputDecimals).toFixed(0),
    ...
    renderSummary(inputError, outputError) {
        ...
        return ( <
            ContextualInfo openDetailsText = {
        ...
        if (lastEditedField === INPUT) {
            ...
            return ( <
                ...
                div > {
                    ...
                    b(`${+outputValue} ${outputLabel}`)
                }. <
                /div> <
                div className = "send__last-summary-text" > {
                    t("itWillCost")
                } {
                    b(`${+maxInput} ${inputLabel}`)
                } {
                    t("orTransFail")
                } <
                /div> < /
    ...
    render() {
        ...
        return ( <
            ...
            /MediaQuery> <
            div className = {
                classnames('swap__content', {
                    'swap--inactive': !this.props.isConnected,
                })
            } >
            <
            NavigationTabs className = {
                classnames('header__navigation', {
                    'header--inactive': !this.props.isConnected,
                })
            }
            /> <
            CurrencyInputPanel title = {
                t("input")
            }
            description = {
                lastEditedField === OUTPUT ? estimatedText : ''
            }
            extraText = {
                this.renderBalance(inputCurrency, inputBalance, inputDecimals)
            }
            onCurrencySelected = {
                inputCurrency => this.setState({
                    inputCurrency
                }, this.recalcForm)
            }
            onValueChange = {
            ...
            <
            img onClick = {
                this.flipInputOutput
            }
            className = "swap__down-arrow swap__down-arrow--clickable"
            src = {
                isValid ? ArrowDownBlue : ArrowDownGrey
            }
            /> < /
            div > <
            /OversizedPanel> <
            ...
            description = {
                lastEditedField === INPUT ? estimatedText : ''
            }
            extraText = {
                this.renderBalance(outputCurrency, outputBalance, outputDecimals)
            }
            onCurrencySelected = {
                outputCurrency => this.setState({
                    outputCurrency
                }, this.recalcForm)
            }
            onValueChange = {
...
const b = text => < span className = "swap__highlight-text" > {
    text
} < /span>;
