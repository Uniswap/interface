...
async function runTypeCheck(ts, baseDir, tsConfigPath, cacheDir) {
    ...
    var ref;
    const firstError = (ref = allDiagnostics.find((d)=>d.category === _diagnosticFormatter.DiagnosticCategory.Error && Boolean(d.file))) != null ? ref : allDiagnostics.find((d)=>d.category === _diagnosticFormatter.DiagnosticCategory.Error);
    if (firstError) {
