...
class CreateExchange extends Component {
    ...
    validate() {
        ...
        const {
            label,
            decimals
        } = selectors().getBalance(account, tokenAddress);
    ...
    onCreateExchange = () => {
        ...
        web3.eth.getGasPrice()
            .then(function(gasPrice) {
                factory.methods.createExchange(tokenAddress).estimateGas({
                        from: account
                    })
    ...
    renderSummary() {
        ...
        if (!tokenAddress) {
            return ( <
                ...
                <
                div className = "create-exchange__summary-text" > {
        ...
        if (errorMessage) {
            return ( <
                div className = "create-exchange__summary-panel" >
                <
                div className = "create-exchange__summary-text create-exchange--error" > {
                    errorMessage
                } < /div> <
                /div>
            )
