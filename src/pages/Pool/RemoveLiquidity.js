...
class RemoveLiquidity extends Component {
  ...
  validate() {
    ...
    const { value: liquidityBalance, decimals: liquidityDecimals } = getBalance(
      account,
      exchangeAddress
    )

    if (
      liquidityBalance.isLessThan(
        BN(value).multipliedBy(10 ** liquidityDecimals)
      )
    ) {
  ...
  renderTransactionDetails = () => {
    ...
    const { value: liquidityBalance, decimals } = getBalance(
      account,
      exchangeAddress
    )
    const { value: ethReserve } = getBalance(exchangeAddress)
    ...
    const minTokenWithdrawn = tokenWithdrawn
      ...
      .toFixed(7)
    const maxTokenWithdrawn = tokenWithdrawn
      .multipliedBy(1 + SLIPPAGE)
      .toFixed(7)
    ...
    return (
      <div>
        <div className="pool__summary-modal__item">
          {t('youAreRemoving')} {b(`${+BN(ethWithdrawn).toFixed(7)} ETH`)}{' '}
          {t('and')}{' '}
  ...
  renderOutput() {
    ...
    const { value: ethReserve } = getBalance(exchangeAddress)
    const { value: tokenReserve, decimals: tokenDecimals, label } = getBalance(
      exchangeAddress,
      tokenAddress
    )
    ...
    const ethPer = ethReserve.dividedBy(totalSupply)
    const tokenPer = tokenReserve
      .multipliedBy(10 ** (18 - tokenDecimals))
      .dividedBy(totalSupply)
    const exchangeRate = tokenReserve
      .multipliedBy(10 ** (18 - tokenDecimals))
      .div(ethReserve)
