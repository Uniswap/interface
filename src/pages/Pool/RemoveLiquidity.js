...
class RemoveLiquidity extends Component {
    ...
    onRemoveLiquidity = async () => {
        ...
        const {
            value: tokenReserve
        } = getBalance(exchangeAddress, tokenAddress);
    ...
    getBalance = () => {
        ...
        const {
            value,
            decimals
        } = selectors().getBalance(account, exchangeAddress);
        if (!decimals) {
            return '';
        }
    ...
    renderSummary(errorMessage) {
        ...
        if (errorMessage) {
            ...
            const {
                label
            } = selectors().getTokenBalance(tokenAddress, fromToken[tokenAddress]);
    ...
    renderTransactionDetails = () => {
        ...
        return ( <
            ...
            div className = "pool__summary-modal__item" > {
                ...
                b(`${+BN(ethWithdrawn).toFixed(7)} OETH`)
            } {
                t("and")
            } {
                b(`${+minTokenWithdrawn} - ${+maxTokenWithdrawn} ${label}`)
            } {
                t("outPool")
            } < /div> <
            div className = "pool__summary-modal__item" > {
    ...
    renderOutput() {
        ...
        const blank = [ <
            ...
            disableUnlock /
            > , <
            OversizedPanel key = "remove-liquidity-input-under"
            hideBottom >
            <
        ...
        return [ <
            ...
            span > {
                `${ethReserve.dividedBy(10 ** 18).toFixed(2)} OETH + ${tokenReserve.dividedBy(10 ** tokenDecimals).toFixed(2)} ${label}`
            } < /span> <
            /div> <
            div className = "pool__exchange-rate-wrapper" >
            <
