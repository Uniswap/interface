...
class AddLiquidity extends Component {
    ...
    getBalance(currency) {
        ...
        const {
            value,
            decimals
        } = selectors().getBalance(account, currency);
        if (!decimals) {
            return '';
        }
    ...
    onAddLiquidity = async () => {
        ...
        const {
            decimals
        } = selectors().getTokenBalance(outputCurrency, fromToken[outputCurrency]);
        const tokenAmount = BN(outputValue).multipliedBy(10 ** decimals);
        const {
            value: ethReserve
        } = selectors().getBalance(fromToken[outputCurrency]);
    ...
    isNewExchange() {
        ...
        const {
            value: tokenValue,
            decimals
        } = selectors().getBalance(fromToken[token], token);
        const {
            value: ethValue
        } = selectors().getBalance(fromToken[token], eth);
    ...
    renderInfo() {
        ...
        const blank = ( <
            ...
            <
            span className = "pool__exchange-rate" > {
                t("exchangeRate")
            } < /span> <
            span > - < /span> <
            /div> <
        ...
        const {
            ...
            decimals,
            label
        } = getBalance(exchangeAddress, token);
        const {
            value: ethValue
        } = getBalance(exchangeAddress);
        const {
            value: liquidityBalance
        } = getBalance(account, exchangeAddress);
        ...
        if (this.isNewExchange()) {
            ...
            return ( <
                ...
                span > {
                    ` ${ethValue.dividedBy(10 ** 18).toFixed(2)} ${eth} + ${tokenValue.dividedBy(10 ** decimals).toFixed(2)} ${label}`
                } < /span> <
                /div> <
                div className = "pool__exchange-rate-wrapper" >
                <
                ...
                /span> <
                span > {
                    `${ownedEth.toFixed(2)} OETH + ${ownedToken.toFixed(2)} ${label}`
                } < /span> <
                /div> <
                /div>
            )
        ...
        return ( <
            ...
            span > {
                `1 OETH = ${tokenValue.multipliedBy(10 ** (18 - decimals)).dividedBy(ethValue).toFixed(4)} ${label}`
            } < /span> <
            /div> <
            div className = "pool__exchange-rate-wrapper" >
            <
    ...
    renderSummary(inputError, outputError) {
        ...
        if (inputError || outputError) {
            ...
            contextualInfo = t("enterCurrencyOrLabelCont", {
                inputCurrency,
                label
            });
        }
        ...
        return ( <
            ContextualInfo key = "context-info"
            openDetailsText = {
    ...
    renderTransactionDetails = () => {
        ...
        } = selectors().getBalance(account, fromToken[outputCurrency]);
        ...
        if (this.isNewExchange()) {
            return ( <
                ...
                div className = "pool__summary-item" > {
                    ...
                    b(`1 OETH = ${BN(outputValue).dividedBy(inputValue).toFixed(4)} ${label}`)
                }. < /div> <
                div className = "pool__summary-item" > {
                    t("youWillMint")
                } {
        ...
        return ( <
            ...
            div className = "pool__summary-modal__item" > {
                ...
                b(`${+BN(inputValue).toFixed(7)} OETH`)
            } {
                t("and")
            } {
                b(`${+minOutput.toFixed(7)} - ${+maxOutput.toFixed(7)} ${label}`)
            } {
                t("intoPool")
            } < /div> <
            div className = "pool__summary-modal__item" > {
            ...
            div className = "pool__summary-modal__item" > {
                ...
                b(+adjTotalSupply.toFixed(7))
            } < /div> <
            div className = "pool__summary-modal__item" > {
                t("tokenWorth")
            } {
                b(+ethReserve.dividedBy(totalSupply).toFixed(7))
            }
            OETH {
                t("and")
            } {
                b(+tokenReserve.dividedBy(totalSupply).toFixed(7))
            } {
                label
            } < /div> <
            /div>
    ...
    render() {
        ...
        return [ <
            ...
            <
            img className = "swap__down-arrow"
            src = {
                isValid ? PlusBlue : PlusGrey
            }
            /> <
            /div> <
            /OversizedPanel> <
            ...
            description = {
                this.isNewExchange() ? `(${t("estimated")})` : ''
            }
            extraText = {
                this.getBalance(outputCurrency)
            }
...
function b(text) {
    return <span className = "swap__highlight-text" > {
        text
    } < /span>
}