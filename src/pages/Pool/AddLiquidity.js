...
class AddLiquidity extends Component {
  ...
  onAddLiquidity = async () => {
    ...
    const { decimals } = selectors().getTokenBalance(
      outputCurrency,
      fromToken[outputCurrency]
    )
    const tokenAmount = BN(outputValue).multipliedBy(10 ** decimals)
    const { value: ethReserve } = selectors().getBalance(
      fromToken[outputCurrency]
    )
    const totalLiquidity = await exchange.methods.totalSupply().call()
    const liquidityMinted = BN(totalLiquidity).multipliedBy(
    ...
    const minLiquidity = this.isNewExchange()
      ? BN(0)
      : liquidityMinted.multipliedBy(1 - MAX_LIQUIDITY_SLIPPAGE)
    const maxTokens = this.isNewExchange()
      ? tokenAmount
      : tokenAmount.multipliedBy(1 + MAX_LIQUIDITY_SLIPPAGE)
  ...
  isNewExchange() {
    ...
    const { inputCurrency, outputCurrency } = this.state
    const eth = [inputCurrency, outputCurrency].filter(
      currency => currency === 'ETH'
    )[0]
    const token = [inputCurrency, outputCurrency].filter(
      currency => currency !== 'ETH'
    )[0]
    ...
    const { value: tokenValue, decimals } = selectors().getBalance(
      fromToken[token],
      token
    )
  ...
  renderInfo() {
    ...
    const { value: tokenValue, decimals, label } = getBalance(
      exchangeAddress,
      token
    )
    ...
    const ownedToken = tokenPer
      .multipliedBy(liquidityBalance)
      ...
      return (
        <div className="pool__summary-panel">
          ...
          <div className="pool__exchange-rate-wrapper">
            ...
            <span>{`${ownedEth.toFixed(2)} ETH + ${ownedToken.toFixed(
              2
            )} ${label}`}</span>
          </div>
        </div>
  ...
  renderSummary(inputError, outputError) {
    ...
    let contextualInfo = ''
    let isError = false
    const { label } = selectors().getTokenBalance(
      outputCurrency,
      fromToken[outputCurrency]
    )
  ...
  renderTransactionDetails = () => {
    ...
    const { decimals: poolTokenDecimals } = selectors().getBalance(
      account,
      fromToken[outputCurrency]
    )
      ...
      return (
        <div>
          <div className="pool__summary-item">
            ...
            {b(`${outputValue} ${label}`)} {t('intoPool')}
          </div>
          <div className="pool__summary-item">
    ...
    const minPercentage = minOutput
      .dividedBy(minOutput.plus(tokenReserve))
      .multipliedBy(100)
    const maxPercentage = maxOutput
      .dividedBy(maxOutput.plus(tokenReserve))
      .multipliedBy(100)
    const liquidityMinted = BN(inputValue).multipliedBy(
      totalSupply.dividedBy(ethReserve)
    )
    ...
    return (
      <div>
        <div className="pool__summary-modal__item">
          {t('youAreAdding')} {b(`${+BN(inputValue).toFixed(7)} ETH`)}{' '}
          {t('and')}{' '}
          {b(`${+minOutput.toFixed(7)} - ${+maxOutput.toFixed(7)} ${label}`)}{' '}
          {t('intoPool')}
        </div>
        <div className="pool__summary-modal__item">
          {t('youWillMint')} {b(+liquidityMinted.toFixed(7))}{' '}
          {t('liquidityTokens')}
        </div>
        <div className="pool__summary-modal__item">
        ...
        <div className="pool__summary-modal__item">
          {t('tokenWorth')} {b(+ethReserve.dividedBy(totalSupply).toFixed(7))}{' '}
          ETH {t('and')} {b(+tokenReserve.dividedBy(totalSupply).toFixed(7))}{' '}
          {label}
        </div>
      </div>
    )
...
export default connect(
  state => ({
    isConnected:
      ...
      state.web3connect.networkId == (process.env.REACT_APP_NETWORK_ID || 1),
