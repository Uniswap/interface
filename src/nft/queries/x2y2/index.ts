...
export const newX2Y2Order = async (payload: OrderPayload): Promise<boolean> => {
  ...
  const url = `${process.env.REACT_APP_TEMP_API_URL}/postX2Y2SellOrderWithApiKey`
...
export const getOrderId = async (collectionAddress: string, tokenId: string): Promise<number | undefined> => {
  const url = `${process.env.REACT_APP_TEMP_API_URL}/getX2Y2OrderId?collectionAddress=${collectionAddress}&tokenId=${tokenId}`
