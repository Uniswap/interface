...
it('data page loads with header showing', () => {
  const { asFragment } = render(
    <DataPage asset={TEST_NFT_ASSET} collection={TEST_NFT_COLLECTION_INFO_FOR_ASSET} showDataHeader={true} />
  )
...
it('data page loads without header showing', () => {
  const { asFragment } = render(
    <DataPage asset={TEST_NFT_ASSET} collection={TEST_NFT_COLLECTION_INFO_FOR_ASSET} showDataHeader={false} />
  )
  expect(asFragment()).toMatchSnapshot()
})
