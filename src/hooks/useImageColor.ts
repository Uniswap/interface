...
const DEFAULT_COLOR = [35, 43, 43]

export function useImageColor(image: string | undefined, checkDistance = false): number[] {
  const [color, setColor] = useState(DEFAULT_COLOR)
  useEffect(() => {
    async function getColor(image: string | undefined) {
      if (image === undefined) {
        setColor(DEFAULT_COLOR)
        return
      }
      if (image in predefinedTokenColors) {
        setColor(predefinedTokenColors[image])
        return
      }
      try {
        ...
        setColor(getAverageColor(arrayBuffer, type, checkDistance))
      } catch (e) {
        setColor(DEFAULT_COLOR)
      }
    }
...
const predefinedTokenColors: { [key: string]: number[] } = {
  // old WBTC
