...
export default function PoolPositionListItem({ positionDetails, returnPage }: PoolPositionListItemProps) {
  ...
  const poolStake = poolDelegatedStake ? (Number(poolDelegatedStake) / 1e18).toFixed(0) : 0
  const aprToString = apr ? (Number(apr) * 100).toFixed(2) : 0
  const poolOwnStakeString = poolOwnStake ? (Number(poolOwnStake) / 1e18).toFixed(0) : 0
  const irrToString = irr ? (Number(irr) * 100).toFixed(2) : 0
