...
import { useGammaHypervisorContract, useMasterChefContract, useMasterChefContracts } from 'hooks/useContract'
import { useFarmingSubgraph } from 'hooks/useIncentiveSubgraph'
import { useSingleCallResult } from 'lib/hooks/multicall'
import { Deposit, FormattedRewardInterface } from 'models/interface/farming'
...
import { useCombinedActiveList, useMultipleContractMultipleData } from 'state/lists/hooks'
import { formatReward, getTokenFromAddress, useUSDCPricesFromAddresses } from 'utils/farmUtils'

import QIGammaMasterChef from '../../../abis/gamma-masterchef1.json'
import { FarmingType, GammaPair, GammaPairs, GlobalConst } from '../constants'
...
export default function FarmingMyFarms({ search, chainId }: { search: string; chainId: number }) {
  ...
  return (
    <Box mt={2}>
      ...
      <Box px={2} my={2}>
        <h6>QuickSwap farms</h6>
      </Box>
      {transferredPositionsLoading || eternalFarmPoolAprsLoading || eternalFarmAprsLoading || !shallowPositions ? (
          ...
          {farmedNFTs && farmedNFTs.length > 0 && chainId && (
            <Box pb={2}>
              ...
              <Box mt={2}>
                {farmedNFTs.map((el, i) => {
                  return (
                    <div className="v3-my-farms-position-card" key={i} data-navigatedto={hash == `#${el.id}`}>
                      <FarmCard
                        chainId={chainId}
                        el={el}
                        poolApr={eternalFarmPoolAprs ? eternalFarmPoolAprs[el.pool.id] : undefined}
                        farmApr={eternalFarmAprs ? eternalFarmAprs[el.farmId] : undefined}
                      />
                    </div>
                  )
                })}
