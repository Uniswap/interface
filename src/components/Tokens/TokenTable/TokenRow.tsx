...
> = {
  ...
  [TokenSortMethod.TOTAL_VALUE_LOCKED]: (
    <Trans>
      Total value locked (TVL) is the aggregate amount of the asset available
      across all Uniswap v3 liquidity pools.
    </Trans>
  ),
...
export const LoadedRow = forwardRef(
  (props: LoadedRowProps, ref: ForwardedRef<HTMLDivElement>) => {
    const { tokenListIndex, tokenListLength, token, sortRank } = props;
    const filterString = useAtomValue(filterStringAtom);

    const filterNetwork = validateUrlChainParam(
      useParams<{ chainName?: string }>().chainName?.toUpperCase()
    );
    ...
    return (
      <div ref={ref} data-testid={`token-table-row-${token.address}`}>
        <StyledLink
          ...
          onClick={() =>
            sendAnalyticsEvent(
              InterfaceEventName.EXPLORE_TOKEN_ROW_CLICKED,
        ...
        >
          <TokenRow
            ...
            tvl={
              <ClickableContent>
                {formatNumber(
                  token.market?.totalValueLocked?.value,
