...
function augmentResponse(response: FloodAPIResponse, chainId: SupportedFloodChainId) {
...
export function useFloodAPI(tokenIn?: string, tokenOut?: string, amountIn?: string, chainId?: number) {
  const enabled =
    Boolean(amountIn) &&
  ...
  const result = useQuery(
    ...
    { enabled, refetchInterval: AVERAGE_L1_BLOCK_TIME, keepPreviousData: true }
  )
  return result
}
