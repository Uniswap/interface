...
describe('#useRoute', () => {
  it('handles an undefined payload', () => {
    const { result } = renderHook(() => useRoutes(undefined, undefined, undefined))
  ...
  describe('with ETH', () => {
    it('outputs native ETH as input currency', () => {
      ...
      expect(result.current).toBeDefined()
      expect(result.current?.length).toBe(1)
      expect(result.current && result.current[0].route.input).toStrictEqual(ETH)
      expect(result.current && result.current[0].route.output).toStrictEqual(USDC)
      expect(result.current && result.current[0].route.tokenPath).toStrictEqual([WETH, USDC])
      expect(result.current && result.current[0].inputAmount.toSignificant()).toBe('1')
      expect(result.current && result.current[0].outputAmount.toSignificant()).toBe('5')
    })
    ...
    it('outputs native ETH as output currency', () => {
      ...
      expect(result.current?.length).toBe(1)
      expect(result.current && result.current[0].route.input).toStrictEqual(USDC)
      expect(result.current && result.current[0].route.output).toStrictEqual(ETH)
      expect(result.current && result.current[0].route.tokenPath).toStrictEqual([USDC, WETH])
      expect(result.current && result.current[0].inputAmount.toSignificant()).toBe('5')
      expect(result.current && result.current[0].outputAmount.toSignificant()).toBe('1')
    })
  })
})
