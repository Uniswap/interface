...
export const routingApi = createApi({
  ...
  baseQuery: fetchBaseQuery({
    ...
    fetchFn: logger.fetch.bind(null, 'swap:api'),
  }),
  endpoints: (build) => ({
      ...
      async queryFn(args, _api, _extraOptions, fetch) {
        ...
        try {
          if (routerPreference === RouterPreference.API) {
            ...
            const { data } = await logger.measure('swap:client', () =>
              getClientSideQuote(
                args,
                router,
          ...
          return { error: { status: 'FETCH_ERROR', error: e.toString() } as FetchBaseQueryError }
        }
      },
