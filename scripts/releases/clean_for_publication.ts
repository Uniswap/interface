 ...
 */
export function parseIgnoreFile(ignoreFilePath: string, deps: FilesystemDeps): Ignore {
 ...
 */
export function cleanForPublication(options: CleanForPublicationOptions = {}): CleanResult {
  ...
  console.log(`Found a total of ${dirs.length} directories and ${files.length} files to delete`)
  ...
  for (const dirPath of sortedDirs) {
    console.log(`${dryRun ? '[DRY RUN] Would delete' : 'Deleting'} directory ${dirPath}`)
    if (!dryRun) {
