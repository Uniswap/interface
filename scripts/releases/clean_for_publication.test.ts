...
describe('clean_for_publication', () => {
  describe('parseIgnoreFile', () => {
    test('should parse ignore patterns from file', () => {
      ...
      expect(mockDeps.readFileSync).toHaveBeenCalledWith('/path/to/.publishignore', 'utf-8')
  ...
  describe('cleanForPublication', () => {
    ...
    test('should use custom ignore file when specified', () => {
      ...
      expect(readFileMock).toHaveBeenCalledWith('/root/.custom-ignore', 'utf-8')
    })
  })
...
`,
    ...
    test('should not delete anything in dry run mode', () => {
      ...
      expect(fs.existsSync(path.join(testDir, 'src', 'Button.stories.tsx'))).toBe(true)
