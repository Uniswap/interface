/**
 * Drives the menu-family type-level drop-in contract (`type-parity.ts`)
...
describe('menu-family compat type-level drop-in contract', () => {
  it('ContextMenu/MenuOptionItem/MenuContent/DropdownMenuSheetItem props are covered (tsc)', () => {
    let output = ''
    try {
    ...
    const contractDiagnostics = output
      ...
      .filter(
        (line) =>
          line.includes('menu/type-parity.ts') ||
          line.includes('menu-compat') ||
          line.includes('components/menus/') ||
          line.includes('dropdownMenuSheet/'),
      )
