 ...
 * TypeScript compiler using `tsconfig.type-parity.view.json`.
 *
 ...
 * file itself — uncovered keys, degraded ViewProps resolution, value-level
...
describe('ViewCompat type-level drop-in contract', () => {
  it('ViewProps is covered up to the documented exclusions (tsc)', () => {
    let output = ''
    try {
      output = execFileSync(tscBin, ['-p', 'tsconfig.type-parity.view.json'], {
        cwd: pkgRoot,
        encoding: 'utf8',
    ...
    const contractDiagnostics = output
      ...
      .filter((line) => line.includes('view/type-parity.ts') || line.includes('view-compat'))
