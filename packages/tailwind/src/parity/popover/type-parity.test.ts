/**
 * Drives the popover-compat type-level drop-in contract (`type-parity.ts`)
...
describe('popover-compat type-level drop-in contract', () => {
  it('AdaptiveWebPopoverContent props are covered up to the documented exclusions (tsc)', () => {
    let output = ''
    try {
    ...
    const contractDiagnostics = output
      ...
      .filter(
        (line) =>
          line.includes('popover/type-parity.ts') ||
          line.includes('popover-compat') ||
          line.includes('AdaptiveWebPopoverContent'),
      )
