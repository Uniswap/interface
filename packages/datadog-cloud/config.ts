...
export function getTeamConfig(team: string): TeamConfig {
  ...
  pulumi.log.warn(`No configuration found for team '${team}', using default team '${settings.defaultTeam}'`)
...
export function buildLinksSection(opts: {
  ...
  const encodedLogQuery = opts.logQuery.replace(/:/g, '%3A').replace(/ /g, '%20')
