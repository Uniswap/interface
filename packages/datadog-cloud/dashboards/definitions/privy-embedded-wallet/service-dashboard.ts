...
export const privyEmbeddedWalletDashboards: DashboardDefinition[] = [
  {
    ...
    description:
      'Embedded wallet service health: request rate, errors, latency (ALB + APM), per-endpoint breakdown, dependency call rates.',
    ...
    widgets: [
      // Row 1: Request rate (ALB) | 5xx error rate (ALB)
      {
      ...
      // Row 3: HTTP status code distribution | Healthy host count
      {
      ...
      // Row 4: APM request rate by resource (top endpoints) | APM apdex by resource
      {
