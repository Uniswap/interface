...
export async function sendDelegatedTransaction(ctx: {
  ...
  const { encoded } = await TradingApiClient.fetchWalletEncoding7702({
    calls: transactions.map((tx) => {
      // The Trading API rejects `calls[].value` unless it matches /^0x[a-fA-F0-9]+$/.
      // Validate before defaulting so a malformed value fails loud at source rather
      // than as an opaque API 400. `isValidHexString` rejects "0" and accepts "0x0".
      if (tx.value !== undefined && !isValidHexString(tx.value)) {
        throw new InvalidDelegatedCallValueError(tx.value)
      }
      return {
