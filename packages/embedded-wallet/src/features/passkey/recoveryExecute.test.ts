...
vi.mock('@universe/embedded-wallet/src/features/passkey/deriveArgon2', () => ({
  deriveArgon2: vi.fn(),
}))

vi.mock('@universe/embedded-wallet/src/features/passkey/pinCrypto', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@universe/embedded-wallet/src/features/passkey/pinCrypto')>()
  return {
    ...actual,
    blindPin: vi.fn(),
    finalizeOprf: vi.fn(),
...
const { encryptAuthKey, generateAuthKeyPair, hashAuthMethodId, blindPin, finalizeOprf } =
  await import('@universe/embedded-wallet/src/features/passkey/pinCrypto')
...
async function buildValidBlob(pin: string): Promise<{ blob: string; authPrivateKey: Uint8Array; salt1: Uint8Array }> {
  ...
  const { AES_KEY_LENGTH, HKDF_INFO } = await import('@universe/embedded-wallet/src/features/passkey/pinCrypto')
...
describe('attemptPinDecryption', () => {
  ...
  it('returns rate_limited when OPRF evaluation returns error', async () => {
    vi.mocked(blindPin).mockResolvedValue({
      ...
      blindState: {
        finalizationData: {},
        client: {},
      } as unknown as import('@universe/embedded-wallet/src/features/passkey/pinCrypto').OprfBlindState,
    })
  ...
  it('returns wrong_pin when GCM decryption fails and calls fetchReportDecryptionResult(false)', async () => {
    ...
    vi.mocked(blindPin).mockResolvedValue({
      blindedElement: 'blinded',
      blindState: {} as unknown as import('@universe/embedded-wallet/src/features/passkey/pinCrypto').OprfBlindState,
    })
    vi.mocked(EmbeddedWalletApiClient.fetchOprfEvaluate).mockResolvedValue({
...
describe('executeRecoveryExport', () => {
  ...
  it('reads exportSigningPayload (camelCase) and returns the exported ciphertext', async () => {
    const authPrivateKey = (
      await (await import('@universe/embedded-wallet/src/features/passkey/pinCrypto')).generateAuthKeyPair()
    ).privateKey
    vi.mocked(EmbeddedWalletApiClient.fetchReportDecryptionResult).mockResolvedValue({
