import { hkdfSha256, zeroBuffers } from '@universe/cryptography'
...
import {
  ...
  parseBlob,
  signWithAuthKey,
} from '@universe/embedded-wallet/src/features/passkey/pinCrypto'
import { base64urlToBase64, utf8ToUint8 } from '@universe/encoding'
...
> {
  try {
    ...
    try {
      ...
      pinKey = await deriveArgon2({ pin, salt1, argon2Params: ARGON2_PARAMS })
      ikm = new Uint8Array(oprfOutput.length + pinKey.length)
      ikm.set(oprfOutput, 0)
      ikm.set(pinKey, oprfOutput.length)
      finalKey = hkdfSha256({ ikm, salt: salt2, info: HKDF_INFO, length: AES_KEY_LENGTH })
...
export function decodeSigningPayload(payload: string): { payloadBytes: Uint8Array; payloadObject: object } {
  ...
  return {
    payloadBytes: utf8ToUint8(payloadJson),
