...
vi.mock('@universe/embedded-wallet/src/features/passkey/signing', () => ({
...
describe('embeddedWalletDelegation', () => {
  ...
  describe('sendDelegatedTransaction', () => {
    ...
    const baseCtx = {
      transactions: [{ to: '0xrecipient', data: '0xcalldata', value: '0x0', gas: '100000' }],
      account: mockAccount,
    ...
    it('throws InvalidDelegatedCallValueError for a non-hex call value and does not call the encoder', async () => {
      await expect(
        sendDelegatedTransaction({
          ...baseCtx,
          // Plain decimal "0" is not a valid 0x-prefixed hex string.
          transactions: [{ to: '0xrecipient', data: '0xcalldata', value: '0' }],
          delegationResult: {
            needsDelegation: true,
            contractAddress: MOCK_DELEGATION_ADDRESS,
            isWalletDelegatedToUniswap: false,
      ...
      ).rejects.toThrow(InvalidDelegatedCallValueError)

      expect(mockFetchWalletEncoding7702).not.toHaveBeenCalled()
    })
