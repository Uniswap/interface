...
import {
  ...
  type EmbeddedWalletCapability,
  type GetCapabilitiesCore,
...
import { NoWalletFoundError, safeJSONStringify } from '@universe/embedded-wallet/src/connection/providerUtils'
import {
...
import {
  ...
  signUserOpWithEmbeddedWallet,
} from '@universe/embedded-wallet/src/connection/userOpSigning'
import {
  checkEmbeddedWalletDelegation,
  type EthTransactionParams,
  sendDelegatedTransaction,
} from '@universe/embedded-wallet/src/features/passkey/embeddedWalletDelegation'
import {
...
export class EmbeddedWalletProvider implements EmbeddedWalletProviderApi {
  listeners: Map<string, Set<Listener>>
  chainId: UniverseChainId
  private readonly getPublicClient: GetViemClient
  private readonly getCapabilitiesCore: GetCapabilitiesCore
  ...
  constructor(deps: EmbeddedWalletProviderDeps) {
    ...
    this.getCapabilitiesCore = deps.getCapabilitiesCore
  }
   ...
   */
  async getCapabilities(requestedChainIds?: string[]): Promise<Record<string, EmbeddedWalletCapability>> {
    const account = this.getAccount()
    if (!account) {
      return {}
    }
    return getEmbeddedWalletCapabilities({
      address: account.address,
      requestedChainIds,
      getCapabilitiesCore: this.getCapabilitiesCore,
    })
  }
   ...
   */
  async getCallsStatus(batchId: string | undefined): Promise<EmbeddedWalletCallsStatus> {
  ...
  async sendCalls(params: WalletSendCallsRequest | undefined): Promise<{ id: string }> {
    ...
    return sendEmbeddedWalletCalls({
      ...
      account: account.address,
      signUserOp: (userOp: RpcUserOperation<'0.8'>) => this.signUserOp(userOp),
      sendTransaction: (transactions: unknown[]) => this.sendTransaction(transactions),
      prepareDelegationAuth: (sender: string, chainId: UniverseChainId) =>
        prepareDelegationAuthorization({
