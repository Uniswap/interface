...
import { getSDKPoolFromPoolInformation, parseRestPosition } from 'uniswap/src/features/positions/parseRestPosition'
...
describe('parseRestPosition', () => {
  ...
  it('parses v4Position with empty hooks array — preserves undefined v4hook semantic', () => {
    const v4PositionNoHook: RestV4Position = {
      poolPosition: mockV4PoolPosition,
      hooks: [] as Hook[],
    } as RestV4Position
    const positionNoHook: RestPosition = {
      chainId: UniverseChainId.Mainnet,
      protocolVersion: ProtocolVersion.V4,
      position: {
        case: 'v4Position',
        value: v4PositionNoHook,
      },
      status: PositionStatus.IN_RANGE,
      isHidden: false,
    } as RestPosition

    const result = parseRestPosition(positionNoHook)
