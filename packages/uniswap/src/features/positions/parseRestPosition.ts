...
import { DYNAMIC_FEE_DATA, type FeeData, type PositionInfo } from 'uniswap/src/features/positions/types'
...
export function getSDKPoolFromPoolInformation({
  ...
  try {
    ...
    logger.error(e, {
      tags: {
        file: 'positions/parseRestPosition.ts',
...
export function parseRestPosition(position?: PlainMessage<RestPosition>): PositionInfo | undefined {
  try {
    ...
    if (positionPayload.case === 'v2Pair') {
      ...
      const sdkHook = hook ?? ZERO_ADDRESS
      const pool = getSDKPoolFromPoolPosition({
        pool: v4Position,
        token0,
        token1,
