...
import extractFiatOnRampDetails from 'uniswap/src/features/activity/extract/extractFiatOnRampDetails'
...
export function parseToTransactionDetails({
  transactions,
  hideSpamTokens,
  nftVisibility,
  ...
  return transactions.reduce((accum: TransactionDetails[], transaction) => {
    ...
    switch (transaction.transaction.case) {
      ...
      case RestTransactionType.UniswapX: {
        const parsed = extractUniswapXOrderDetails(transaction.transaction.value)
        if (parsed) {
          accum.push(parsed)
        }
        break
      ...
      case RestTransactionType.FiatOnRamp: {
        const parsed = extractFiatOnRampDetails(transaction.transaction.value)
        if (parsed) {
          accum.push(parsed)
        }
        break
