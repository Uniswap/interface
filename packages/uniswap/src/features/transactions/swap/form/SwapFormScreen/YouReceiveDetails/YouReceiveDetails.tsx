...
export function YouReceiveDetails({
  ...
  const minimumAmount = handleGetMinimumAmount({
      ...
      >
        ...
        <HeightAnimator open={isOpen}>
          ...
          <Flex gap="$spacing12">
            ...
            <RouteDisplay isBridge={isBridge} isUniswapXContext={isUniswapXContext} loading={!swapTxContext.trade} />
          </Flex>
        </HeightAnimator>
      </Flex>
    </TouchableArea>
...
/**
 * Handle getting the minimum amount for a trade.
 * This wraps the trade.minimumAmountOut function to handle errors.
 * Sometimes the trade is an object instead of ClassicTrade, unknown why.
 */
function handleGetMinimumAmount(input: {
  ...
  if (typeof input.trade.minimumAmountOut !== 'function') {
    logger.warn('YouReceiveDetails.web.tsx', 'handleGetMinimumAmount', 'trade.minimumAmountOut is not a function', {
      extra: {
        trade: input.trade,
      },
    })
  ...
  return input.trade.minimumAmountOut(slippageToleranceToPercent(input.slippageTolerance))
}
