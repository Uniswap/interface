...
export function useTokenApprovalInfo(params: TokenApprovalInfoParams): {
  ...
  const { approvalGasFeeResult, revokeGasFeeResult } = useMemo(() => {
  ...
  }, [activeGasStrategy, data?.cancelGasFee, data?.gasEstimates, data?.gasFee, isLoading, tokenApprovalInfo.action])
