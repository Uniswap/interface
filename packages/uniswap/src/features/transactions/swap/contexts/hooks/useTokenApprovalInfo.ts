...
export interface TokenApprovalInfoParams {
  ...
  account?: AccountMeta
  /** When set (e.g. AGGREGATOR with methodParameters.to), passed to check_approval so backend can return approval for this spender or null if already approved */
...
export function useTokenApprovalInfo(params: TokenApprovalInfoParams): ApprovalTxInfo {
  const { account, chainId, wrapType, currencyInAmount, currencyOutAmount, routing, approvalSpender } = params

  const isWrap = wrapType === WrapType.Wrap || wrapType === WrapType.Unwrap
  ...
  const approvalRequestArgs: ApprovalRequest | undefined = useMemo(() => {
    ...
    return {
      ...
      ...(approvalSpender != null && approvalSpender !== '' ? { spender: approvalSpender } : {}),
    }
  }, [
    activeGasStrategy,
    address,
  ...
  return useMemo(() => {
    ...
    const revokeFee = noRevokeNeeded ? '0' : data?.cancelGasFee ?? '0'
