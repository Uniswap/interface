...
import { Route as V2RouteSDK } from '@ring-protocol/v2-sdk'
import { Route as V3RouteSDK } from '@ring-protocol/v3-sdk'
import { Route as V4RouteSDK } from '@ring-protocol/v4-sdk'
import { Route as FewV2RouteSDK } from '@ring-protocol/few-v2-sdk'
import { BridgeQuoteResponse, ClassicQuoteResponse, DutchQuoteResponse, DutchV3QuoteResponse, PriorityQuoteResponse, AggregatorQuoteResponse } from 'uniswap/src/data/apiClients/tradingApi/TradingApiClient'
...
> extends RouterSDKTrade<TInput, TOutput, TTradeType> {
  ...
  constructor({
    ...
    // Exclude FewV2 routes from the RouterSDKTrade constructor since Router SDK
    // does not recognize FewV2 pool types. We still accept them in the
    // ClassicTrade signature for potential future use or analytics, but they are
    // intentionally not passed to the base Trade.
    super({ fewV2Routes, v2Routes, v3Routes, mixedRoutes, v4Routes, tradeType })
    this.quote = quote
...
export class AggregatorTrade {
  quote: AggregatorQuoteResponse
  inputAmount: CurrencyAmount<Currency>
  outputAmount: CurrencyAmount<Currency>
  executionPrice: Price<Currency, Currency>

  tradeType: TradeType
  readonly routing = Routing.AGGREGATOR
  readonly indicative = false
  readonly swapFee?: SwapFee
  ...
  constructor({ quote, currencyIn, currencyOut, tradeType, deadline }: { quote: AggregatorQuoteResponse, currencyIn: Currency, currencyOut: Currency, tradeType: TradeType, deadline?: number }) {
    this.quote = quote
    this.swapFee = getSwapFee()
    this.deadline = deadline ?? (Math.floor(Date.now() / 1000) + 60 * 20) // Default 20 mins
    ...
    if (!quoteInputAmount || !quoteOutputAmount) {
      throw new Error('Error parsing Enso quote currency amounts')
    }
    ...
    // priceImpact from backend is a percentage value (e.g., 0.5 means 0.5%)
    // Convert to Percent: multiply by 100 to get basis points, then divide by 10000
    this.priceImpact = quotePriceImpact !== undefined && quotePriceImpact !== null
  ...
  minimumAmountOut(_slippageTolerance: Percent, _amountOut?: CurrencyAmount<Currency>): CurrencyAmount<Currency> {
    // If quote has aggregatedOutputs with minAmount, use it?
    // The user provided example has aggregatedOutputs with minAmount.
    // For now, return outputAmount as fallback or use logic similar to BridgeTrade (no slippage calculation on client side as it's pre-calculated)
    // Actually, if we want to respect the 'slippageTolerance' passed in argument, we might need to calculate.
    // But since methodParameters are fixed, the minAmount is fixed.
...
export type Trade<
  ...
  TOutput extends Currency = Currency,
  TTradeType extends TradeType = TradeType,
> = ClassicTrade<TInput, TOutput, TTradeType> | UniswapXTrade | BridgeTrade | AggregatorTrade
