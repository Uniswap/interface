...
async function getAddressFetch(
  name: string,
  provider: providers.JsonRpcProvider,
) {
...
async function getAvatarFetch(
  ...
  const checkedName = areAddressesEqual({
    ...
    ? name
    : null;
  const avatarUri = checkedName ? await provider.getAvatar(checkedName) : null;
  // Sanitize avatar URI to only allow safe protocols (http:// and https://)
  return sanitizeAvatarUri(avatarUri);
}
...
async function getOnChainEnsFetch(
  params: EnsLookupParams,
): Promise<string | null> {
...
export function useENSName(address?: Address) {
  ...
  return useEnsQuery(
    EnsLookupType.Name,
    isValidEVMAddress ? address : undefined,
  );
}
