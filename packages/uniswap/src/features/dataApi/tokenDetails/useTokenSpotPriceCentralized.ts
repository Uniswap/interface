...
export function useTokenSpotPriceCentralized(
  ...
  const legacyPrice = useTokenSpotPriceLegacy(currencyId, options)

  return options?.preferProjectMarketData ? legacyPrice : isSupported ? centralizedPrice : legacyPrice
}
