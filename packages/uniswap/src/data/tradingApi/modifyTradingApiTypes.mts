...
// Add aggregators to QuoteRequest
modifyType(quoteRequestFile, 'QuoteRequest', [{ name: 'aggregators', type: 'string[]', isOptional: true }])

// Add optional spender to ApprovalRequest (for aggregator: approve to methodParameters.to; backend checks allowance for this spender)
modifyType(approvalRequestFile, 'ApprovalRequest', [{ name: 'spender', type: 'string', isOptional: true }])
...
// Add methodParameters to ClassicQuote
modifyType(classicQuoteFile, 'ClassicQuote', [
  { name: 'methodParameters', type: '{ to: string; calldata: string; value: string }', isOptional: true },
])

// Add AGGREGATOR to Routing enum
const routingFile = project.addSourceFileAtPath(`${path}/Routing.ts`)
const routingEnum = routingFile.getEnum('Routing')
if (routingEnum) {
  if (!routingEnum.getMember('AGGREGATOR')) {
    routingEnum.addMember({
      name: 'AGGREGATOR',
      value: 'AGGREGATOR',
    })
    console.log('Added AGGREGATOR to Routing enum')
  } else {
    console.log('AGGREGATOR already exists in Routing enum')
  }
} else {
  console.log('Enum Routing not found')
}
