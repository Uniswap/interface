...
// Wrap RNGH Pressable with createAnimatedComponent so Reanimated 4 strict mode accepts the animated style.
const AnimatedPressable = Animated.createAnimatedComponent(Pressable)

// Native base is RNGH's Pressable so press hit-testing reads the native view frame. Tamagui's
// measure-based press on a plain View collapses the touch target to its content on Android Fabric,
// leaving the styled padding (the visible button area) untappable (RN #51621). Web keeps the XStack base.
const CustomButtonFrameWithoutCustomProps = styled(AnimatedPressable, {
  context: buttonStyledContext,
  name: 'Button',
  // Tamagui handles `group` at runtime on any base, but its type isn't inferred for a non-Tamagui (RNGH) base.
  // @ts-expect-error group is a valid Tamagui styled config key
  group: 'item',
  '$platform-web': {
